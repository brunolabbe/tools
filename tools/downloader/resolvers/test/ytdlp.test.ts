...
import { AppError, mediaVariantSchema, probeResultSchema } from "@downloader/contract";
...
/** Sets each named key to JSON `null`, as yt-dlp does, on an object typed without it. */
function nullify(target: object | undefined, keys: readonly string[]): void {
  if (target === undefined) return;
  for (const key of keys) Reflect.set(target, key, null);
}

/**
 * An extractor that reports `fps`, and every bitrate and size estimate, as JSON
 * `null` on each format of a progressive ladder (dl-62).
 *
 * `extractor-null-fps.json` is real output, captured from yt-dlp 2025.09.26
 * against a site-specific extractor and reduced to the fields `YtDlpInfo` and
 * `YtDlpFormat` declare, with the page's id, title, host and every URL
 * rewritten. Captured rather than written for the same reason as
 * `generic-null-vcodec.json` above: the value under test is `null` versus an
 * absent key. `fps`, `tbr` and `filesize_approx` arrive `null`; `acodec` and
 * `filesize` do not arrive at all.
 *
 * The probe never failed on it. Its variants carried `fps: null` into the job
 * store, whose read back through `jobSchema` rejected them, so every download
 * from that page ended in `INTERNAL`, "A stored job could not be read", two
 * seconds after it was accepted.
 */
describe("an extractor that reports fps as JSON null", () => {
  const WATCH = "https://vod.example.com/watch/clip";
  const probe = mapYtDlpInfo(fixture("extractor-null-fps"), WATCH, "yt-dlp", {});

  test("every variant passes the schema a stored job is read back through", () => {
    expect(probe.variants).toHaveLength(14);
    for (const variant of probe.variants) {
      expect(mediaVariantSchema.safeParse(variant).error).toBeUndefined();
    }
    expect(probeResultSchema.safeParse(probe).error).toBeUndefined();
  });

  test("a null fps is absent from the variant and from its label", () => {
    expect(probe.variants.filter((variant) => "fps" in variant)).toEqual([]);
    expect(probe.variants.find((variant) => variant.id === "480p")?.label).toBe("480p");
    expect(probe.variants.find((variant) => variant.id === "av1-480p")?.label).toBe("480p · AV1");
  });

  test("a format that names no audio codec is unverified, not silent", () => {
    // The captured files carry AAC, measured with ffprobe; yt-dlp just does not
    // say so. `false` here is what made the picker print "no audio".
    const plain = probe.variants.find((variant) => variant.id === "480p");
    expect(plain?.hasVideo).toBe(true);
    expect(plain).not.toHaveProperty("hasAudio");
  });

  test('"none" is still the answer that means no audio', () => {
    const info = fixture("extractor-null-fps");
    for (const format of info.formats ?? []) format.acodec = "none";

    const silent = mapYtDlpInfo(info, WATCH, "yt-dlp", {});

    expect(silent.variants.map((variant) => variant.hasAudio)).toEqual(
      silent.variants.map(() => false),
    );
  });

  test("a null in any field the mapper copies still maps to a valid probe", () => {
    // Not measured, on purpose: this is the backstop for the next extractor's
    // null, which will arrive in a field nobody has observed null yet.
    const info = fixture("youtube-like");
    nullify(info, ["thumbnail", "title", "webpage_url"]);
    nullify(info.http_headers, ["Referer"]);
    for (const format of info.formats ?? []) {
      ...
      nullify(format.http_headers, ["Referer"]);
    }
    for (const entries of Object.values(info.subtitles ?? {})) {
      for (const entry of entries) nullify(entry, ["name"]);
    }
    for (const entries of Object.values(info.automatic_captions ?? {})) {
      for (const entry of entries) nullify(entry, ["url"]);
    }

    const mapped = mapYtDlpInfo(info, SOURCE.href, "yt-dlp", {});

    expect(mapped.variants.length).toBeGreaterThan(0);
    expect(mapped.subtitles.filter((track) => track.autoGenerated)).toEqual([]);
