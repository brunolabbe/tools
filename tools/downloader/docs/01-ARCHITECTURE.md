...
| `EGRESS_CA_FILE`              | system store | extra CA bundle, **merged** with the public roots; three clients get it — the proxy, the undici dispatcher behind every probe, and ffmpeg when interception is off. **Not Chromium and not yt-dlp**, which verify against their own trust stores behind the tunnelling proxy; since `dl-34` they say `TLS_VERIFICATION_FAILED` when that bites, and boot warns which tiers are covered. `FFMPEG_CA_FILE` is the deprecated spelling and still works, warning at boot |
