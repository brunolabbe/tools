...
- **verified** · the judgment call — does removing the list transfer the trap to `GET /api/jobs/:id`? Checked all three of the builder's reasons independently; concur it does not transfer, no escalation:
  1. No route answers without an id: `JobStore.list` has zero production callers in `api/src` or `engine/src` (the only caller anywhere is `api/test/pipeline.test.ts:525 "store.list().jobs[0]?.status"`; `api/src/db/job-store.ts:241 "list({ limit = 50, offset = 0 } = {})"` is the only other hit, its own definition).
