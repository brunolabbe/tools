...
`docs/02-DEPLOYMENT.md:217` "see the security posture in" already sends a
...
- **Not the store.** `JobStore.list` stays. It is a DB primitive, not a surface,
  and it has a live caller that is not the route: `pipeline.test.ts:525` reads
...
| 2 — whatever that option implies is built and tested                                                               | **proven** — `api/test/routes.test.ts:461 "GET /api/jobs is a route miss, with nothing supplied"`, `:492 "the method was removed, not the path"`, `:506 "reading one job still carries it"`; `downloader/api/test/web-serving.test.ts:92 "GET /api/jobs is a typed 404 here too"`; `api/test/rate-limit.test.ts:249 "POST /api/jobs is limited, and reading a job is not"`; `web/test/mock-api.test.ts:281 "a completed job's link is on getJob, now the only way to reach one"`; the contract and client removals are compiler-enforced (`npm run check` exit 0, re-run) |
| 3 — a test proves an unauthenticated `GET /api/jobs` no longer returns a job it should not, through the real stack | **proven** — `api/test/routes.test.ts:461 "GET /api/jobs is a route miss, with nothing supplied"` (`server.inject` against the real harness with a genuinely completed job in the store) and `downloader/api/test/web-serving.test.ts:92 "GET /api/jobs is a typed 404 here too"` (same request under the container's `WEB_DIR` configuration, where an unmatched path would otherwise hit the SPA fallback before a 404 handler)                                                                                                                                         |
...
- **verified** · `ROUTES.jobs` retained correctly: `app.post(ROUTES.jobs, …)` remains at `api/src/routes/jobs.ts@95c6403:37 "app.post(ROUTES.jobs"`; `app.get(ROUTES.jobs, …)` is gone. `api/test/routes.test.ts` 40/40 pass.
- **verified** · Done-when 3's stronger form: the five `not.toContain` literals (token, id, `SOURCE_URL`, `"cdn.example"`, `"video.mp4"`) traced to their real fixture sources (`api/test/helpers.ts:22 "export const SOURCE_URL ="`, `:28 "cdn.example/master.m3u8"`, `:160 "filename:"`) rather than assumed to match by inspection, and the 404 body confirmed non-trivial (`downloader/api/src/server.ts@20c8fd1:620-624 "server.setNotFoundHandler("`'s `NOT_FOUND` carries `details.path`), so the negative assertions have real content to fail against.
- **verified, method recorded** · test counts are genuinely net-zero, not coincidentally so: built a **second, independent worktree at `origin/main` (`c37cab9`)** — farmed with `worktree-farm.sh`, built with `npm run build`, then ran both suites there rather than trusting the branch's own numbers or the ticket's Log. Baseline: 62 files / 979 tests (downloader), 122 / 1966 (full monorepo). This branch: identical on both counts, both suites. Read the diff of all four touched test files (`api/test/routes.test.ts`, `api/test/rate-limit.test.ts`, `web/test/mock-api.test.ts`, `web/test/app.test.tsx`, plus additions in `api/test/web-serving.test.ts`): no assertion was weakened, and `api/test/rate-limit.test.ts:249 "POST /api/jobs is limited, and reading a job is not"` was strengthened (reads a specific job by id instead of the removed list — a tighter proof of "what it already started").
- **verified** · disclosed weakness 1 (`api/test/routes.test.ts:492 "the method was removed, not the path"`, the POST guard, green in both directions "by construction"): reproduced the mutation check independently — repointed `app.post(ROUTES.jobs, …)` at `/api/jobs-mutated` in a scratch edit, re-ran: `AssertionError: expected 404 to be 201` at line 502, matching the builder's own report exactly. Reverted; `git status` clean after. Judged sound: the assertion does not depend on `GET /api/jobs`'s existence, so mutating what it actually depends on is the only way to get a real red state from it.
- **verified** · disclosed weakness 2 (`api/test/routes.test.ts:461 "GET /api/jobs is a route miss, with nothing supplied"`'s five `not.toContain` lines "never exercised red"): reproduced independently — restored `origin/main`'s `api/src/routes/jobs.ts` in a scratch edit, re-ran: fails at the status-code assertion (`expected 200 to be 404`) and stops there, so the five lines are genuinely never reached in that red run. Reverted; clean after. The compensating positive test at `api/test/routes.test.ts:506 "reading one job still carries it"` (same five strings asserted present on `GET /api/jobs/:id`) is judged sound, and confirmed further than reading it: the five literals independently trace to real fixture values on both sides (not independently-typed strings that happen to coincide), so a typo in either list would in fact have been caught.
- **verified** · the judgment call — does removing the list transfer the trap to `GET /api/jobs/:id`? Checked all three of the builder's reasons independently; concur it does not transfer, no escalation:
  1. No route answers without an id: `JobStore.list` has zero production callers in `api/src` or `engine/src` (the only caller anywhere is `api/test/pipeline.test.ts:525 "store.list().jobs[0]?.status"`; `api/src/db/job-store.ts@95c6403:241 "list({ limit = 50, offset = 0 } = {})"` is the only other hit, its own definition).
  2. Job ids are `randomUUID()` v4 (`api/src/routes/jobs.ts:20 "import { randomUUID }"`, `api/src/routes/jobs.ts@95c6403:57 "id: randomUUID(),"`), 122 bits, asserted by `api/test/routes.test.ts:530 "a job id is not guessable"`'s regex (passing); the same id already reaches `result.downloadUrl` via the same route (`api/test/routes.test.ts:506 "reading one job still carries it"`).
  3. `web/src/hooks/useJobs.ts:112 "refetch: async (id) => (await api.getJob(id)).job"` and `:136 "const { job } = await api.getJob(jobId);"` both call `api.getJob` for real, live product paths — the SSE-drop reconcile (`attach`'s `refetch`) and the on-mount restore effect that merges job state, including `result.downloadUrl`, into what the UI renders. Read the surrounding code at both sites, not just the cited line.
- **verified** · `JobStore.list` correctly kept as a DB primitive, not a surface: zero production callers, one live test caller (`api/test/pipeline.test.ts:525 "store.list().jobs[0]?.status"`).
- **verified** · `docs/02-DEPLOYMENT.md` correctly untouched (`git diff origin/main...HEAD -- docs/02-DEPLOYMENT.md` empty); its "no authentication anywhere in the application" claim (`docs/02-DEPLOYMENT.md@1835657:217 "no authentication anywhere in the application"`) is unchanged and still accurate.
- **dropped** · reviewer initially raised the file location of `docs/02-DEPLOYMENT.md` (root `docs/` rather than `tools/downloader/docs/`) as a possible layout inconsistency, based on reading only its first ~150 lines. Retracted on the builder's correction, independently re-verified: the file is 530 lines, 27 mentions of "planner" and 20 of "downloader", with a 187-line planner-only section (`docs/02-DEPLOYMENT.md@5f6e92c:287-473` "Grounding the planner: a routing engine and a geocoder") and an explicit `## Adding the second tool` section (`docs/02-DEPLOYMENT.md@5f6e92c:474` "Adding the second tool") contrasting the two tools' deployment and Access-policy differences line by line. It is genuinely repo-wide (one tunnel serving both tools) and the sibling of `adr/004`. Not a finding, and the reviewer's own error, not the builder's.
...
- **2026-09-05** — Built option D. `GET /api/jobs` is gone, with
  ...
  **What the brief had wrong or did not anticipate:**
  ...
  - **The Build section said "the client interface, its mock and its tests all
    ...
    tested is covered at the store, `api/test/job-store.test.ts:289`), and
...
when` rather than left implicit.
  - **`JobStore.list` was kept**, and the ticket says nothing either way. It is
    not a surface, and it is not dead: `pipeline.test.ts:525` needs it to read
  ...
  **Acceptance, line by line:**
  ...
  | 2 — what the option implies is built and tested                                                                    | **pass** | `api/test/routes.test.ts:461` · `api/test/routes.test.ts:492` · `downloader/api/test/web-serving.test.ts:92` · `api/test/rate-limit.test.ts:249` · `web/test/mock-api.test.ts:281`. The contract and client removals are enforced by the compiler, not by a test: `npm run check` fails on a `listJobs` that comes back. |
...
- **2026-09-12 — repo-39: the `## Review` citations anchored, 29 failing references down to 3.** Most were unchanged and needed only an anchor. Four things worth keeping. The `api/src/routes/jobs.ts` id citation was a comma list hiding a second coordinate, and is now two citations. The `server.ts` `NOT_FOUND` pointer did not name the not-found handler even at this record's own commit — that line was a route registration — and is repointed to the handler, where the claim was right. The "no authentication anywhere" citation was a bare shorthand bound to `pipeline.test.ts` when it meant `docs/02-DEPLOYMENT.md`, and is qualified and repointed to where that sentence stands now. And the Why's and Log's copies of the moved pointers were repointed alongside. `GRANDFATHERED` 29 → 3.

  **Left failing:** the dropped finding's description of `docs/02-DEPLOYMENT.md` at review time — the planner section's span and the "Adding the second tool" heading. They were shorthands bound to the wrong file and are qualified now, but they measure a 530-line document that has since grown to 950 lines and moved both headings, so they wait for repo-35's pin.

- **2026-09-13 — repo-44: repo-39's 3 pinned.** The dropped finding measured the deployment document at review time, and `5f6e92c`, the commit that merged this record, holds the same lines. Its 187-line span was two citations ending on a blank line, which cannot take an anchor; it is now one anchored range covering the same span.
