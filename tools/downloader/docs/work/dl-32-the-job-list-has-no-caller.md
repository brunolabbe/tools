...
- `npm run check` exit 0. `npm run format` leaves the tree clean. `node scripts/status.mjs --json` exit 0, `problems: []`, `dl-32` shows `status: done`. `node scripts/citations.mjs tools/downloader/docs/work/dl-32-the-job-list-has-no-caller.md`: 18/18 resolve (re-run; higher-value citations additionally hand-verified for content, not just line resolution). `git merge-tree --write-tree c6c40fd b6969dc8e67810a6ea26e6bb9c7f118eed715a78` (this branch vs. PR #147's head): exit 0, no conflict — confirmed the two branches' `01-ARCHITECTURE.md` edits land in non-overlapping sections (env-var table around line 132 vs. this ticket's security-posture addition at the file's end, line 245+).
