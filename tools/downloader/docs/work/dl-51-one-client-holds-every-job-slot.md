...
| Done when                                                                   | Proof                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| --------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Step-1 test fails on `origin/main`, passes on the branch, Log records both  | `tools/downloader/api/test/per-client-caps.test.ts@04c2fb7:102 "toEqual([201, 201, 429, 429, 429])"` ✓ — verified: the same body run against the `95c6403` api/src failed `expected [ 201, 201, 201, 201, 201 ] to deeply equal [ 201, 201, 429, 429, 429 ]`, and is green at `04c2fb7`. The Log records both                                                                                                                                                                  |
| Over `MAX_JOBS_PER_CLIENT` gets `RATE_LIMITED`, a second client is admitted | `tools/downloader/api/test/per-client-caps.test.ts@04c2fb7:119-137 "MAX_JOBS_PER_CLIENT gets a well-formed RATE_LIMITED"` ✓ · `:140 "createJob(harness, clientB)).statusCode"` ✓                                                                                                                                                                                                                                                                                               |
...
| Count released after completion, failure, cancel and timeout                | completion `tools/downloader/api/test/per-client-caps.test.ts@04c2fb7:200-221 "released after a job completes"` ✓ (see low below) · failure `:227-247 "released after a job fails"` ✓ · timeout `:272 "finished.error?.code).toBe("` ✓ · cancel while running `:284-318 "released when a running job is canceled"` ✓ · cancel while waiting `:356 "jobClientGate.count(clientKey(clientA))).toBe(1)"` ✓ · shutdown `:389 "jobClientGate.count(clientKey(clientA))).toBe(0)"` ✓ |
| The probe route has the equivalent per-client cap, with its own test        | `tools/downloader/api/test/per-client-caps.test.ts@04c2fb7:400-455 "MAX_PROBES_PER_CLIENT gets a well-formed RATE_LIMITED"` ✓ · `:448 "expect(other.statusCode).toBe(200)"` ✓                                                                                                                                                                                                                                                                                                  |
...
- **med** · `tools/downloader/api/src/routes/jobs.ts:81 "const releaseJobSlot = context.jobClientGate.tryAcquire(key);"` takes the per-client slot before `tools/downloader/api/src/routes/jobs.ts@04c2fb7:96 "const job = context.store.create({"` and `tools/downloader/api/src/routes/jobs.ts@04c2fb7:104 "context.queue.enqueue({"`, and nothing releases it if either throws: the only release is the `onSettle` handed to `enqueue`. Reproduced with a gate spec, not committed: `maxJobsPerClient: 2`, `context.store.create` replaced by a function throwing `new AppError("DISK_FULL")`, two POSTs from one address (both 507), `create` restored, then a valid POST from that address answers `429` with `jobClientGate.count` at 2. Two storage failures lock that client out of job creation until restart, which step 5 of the brief calls worse than no cap. `enqueue` also throws once `close()` has begun, reachable across the `tools/downloader/api/src/routes/jobs.ts:57 "await context.guard.assertAllowed"` suspension, but only during shutdown.
- **low** · two findings, one mechanism: duplicating either waiting-path release, `tools/downloader/api/src/jobs/queue.ts:114 "removed?.task.onSettle?.();"` or `tools/downloader/api/src/jobs/queue.ts:127 "entry?.task.onSettle?.();"`, turns no spec red in `per-client-caps.test.ts` or `queue-and-shutdown.test.ts`. The queue specs for those two paths assert a boolean (`tools/downloader/api/test/queue-and-shutdown.test.ts@04c2fb7:332-358 "fires for a task canceled while it was still waiting"`, `tools/downloader/api/test/queue-and-shutdown.test.ts@04c2fb7:394-415 "fires for every waiting task dropped at shutdown"`), so the documented exactly-once is pinned on four of six exit paths. Harmless today only because the release is idempotent.
...
- **low** · the completion spec waits on `tools/downloader/api/test/per-client-caps.test.ts@04c2fb7:210-212 "job to finish"`, whose predicate accepts `failed` as well as `completed`, so that row would stay green if the completion path regressed into failure. Unverified here whether this spec completes today; the same stub completes in `pipeline.test.ts`.
