...
| Loading the page                 | `navigate(page, url, …)`, [`browser.ts:229 "navigate(page, url"`](../../resolvers/src/resolvers/browser.ts)              | "Loading the page…"          |
| Provoking playback               | `provokePlayback`, [`browser.ts:236 "provokePlayback(page"`](../../resolvers/src/resolvers/browser.ts)                   | "Provoking playback…"        |
| Waiting for network quiet        | `waitForQuiet`, [`browser.ts:242 "waitForQuiet({"`](../../resolvers/src/resolvers/browser.ts)                            | "Waiting for the network…"   |
| Settling outstanding requests    | `collector.settle`, [`browser.ts:252 "collector.settle("`](../../resolvers/src/resolvers/browser.ts)                     | nothing                      |
| Fetching the manifest            | `#loadManifest`, [`browser.ts:314 "this.#loadManifest(context"`](../../resolvers/src/resolvers/browser.ts)               | nothing                      |
| Parsing the manifest             | `#parseManifest`, [`browser.ts:317 "this.#parseManifest(hit, text)"`](../../resolvers/src/resolvers/browser.ts)          | nothing                      |
| Weighing a rendition             | `measureVariantSizes` (dl-30), [`browser.ts:337 "await measureVariantSizes("`](../../resolvers/src/resolvers/browser.ts) | nothing                      |
| Trying yt-dlp                    | `runProcess`, [`ytdlp.ts:248 "await runProcess("`](../../resolvers/src/resolvers/ytdlp.ts)                               | nothing                      |
| Trying the direct URL            | `#head`, [`direct.ts:131 "async #head("`](../../resolvers/src/resolvers/direct.ts)                                       | nothing                      |
...
2. Emit from the points in the table. [`registry.ts:82 "for (const resolver of candidates)"`](../../resolvers/src/registry.ts)
...
| Done when                                                                                    | Proof                                                                                                                                                                                                                                                                        |
| -------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A probe answered by the first tier never reports the second or third                         | `api/test/probe-stages.test.ts:253-266 "a probe answered by the first tier never reports the second or third"` (asserts at :264,265,266) ✓, mirrored at `resolvers/test/registry.test.ts@95c6403:351 "a probe answered by the first tier never reports the second or third"` |
| A probe held at the pool semaphore reports waiting for a slot, not "opening a browser"       | `resolvers/test/browser/pool-stages.test.ts:58-61 "The queued lease has announced the wait"` ✓                                                                                                                                                                               |
| A stage not yet reached is absent from the document, not present-and-greyed; fails on `main` | `web/test/chrome.test.tsx:99-123 "a stage the probe has not reached is absent from the document"` (asserts at :105,116,119) ✓ — "fails on `main`" independently reproduced, see Findings                                                                                     |
| The text a screen reader is given changes when a stage advances                              | `web/test/chrome.test.tsx:125-162 "the text a screen reader is given changes when a stage advances"` (asserts at :140 for `aria-live="polite"`, :157-158 for the content change) ✓                                                                                           |
| A job that took `downloading → probing` still shows both where it is and how far it got      | `web/test/job-card.test.tsx:339-373 "a re-probe keeps Downloading marked done"` (asserts at :362,372,373) ✓ — pre-existing dl-9/dl-18 test, file untouched by this branch, still green                                                                                       |
| The indeterminate bar animates, reduced-motion fallback asserted                             | `web/test/styles.test.ts:45-56 "the indeterminate bar animates, by a keyframes block that exists"` (:48,54-55) and `:67-75 "reduced motion stops the travel"` (:70,74) ✓                                                                                                     |
...
- Reverting the `if (!existing.ended)` guard (`api/src/probe-stages.ts:223 "if (!existing.ended)"`) and re-running `api/test/probe-stages.test.ts:89 "reaches a subscriber that only arrives afterwards"` turns it red — `expected 1 to be +0`.
...
**Invariants walked:** no tool-to-tool import; `AppError` with `RATE_LIMITED` (core, correctly) used for both the probe gate and the SSE hub's cap; no shell; no `any`/`console`; `import type` and `.ts` extensions throughout; no new workspace dependency, no Dockerfile edit needed or made. `redactHeaders`/`redactUrl` — `api/src/routes/probe.ts@95c6403:162 "requestContext: probe.requestContext"` passes `probe.requestContext` to the logger unmodified, but `logger.ts`'s `safeFields` (`tools/downloader/api/src/logger.ts:74-112 "function isRequestContext"`) structurally redacts any top-level `requestContext` key regardless of call site, and `tools/downloader/api/test/logging.test.ts:253 "a session cookie in a probe's context never reaches the probe route's log line"` proves a cookie in a probe's context never reaches this exact log line (re-run: 1 passed). No faked progress. `npm run check` passes at `616c0c4`.
...
afterwards"`, fixed at `api/src/probe-stages.ts:223 "if (!existing.ended)"`. (2) `.progress::-webkit-progress-bar` sets
...
- **2026-09-12 — repo-39: the `## Review` citations anchored, 19 failing references down to 0, and the `GRANDFATHERED` entry deleted.** Two shorthand defects. The `safeFields` range was a bare shorthand bound to `routes/probe.ts` when it names `logger.ts`, and is now qualified. The `contract-schemas.test.ts` citation was a comma list whose second and third coordinates nobody had checked; it is expanded to the three `describe` blocks the claim names, and widened from their heading lines because a `describe` heading carries a double quote an anchor cannot. The pool-stages assertion's own text occurs twice in its file, so that citation is widened to the comment above it. Outside the gate record, the Why's emit-point table is repointed symbol by symbol, and the Log's "fixed at" guard coordinate with it. The Log's quoted draft-record claim about `routes/probe.ts` is left as the draft said it.
