...
| Done when                                                                                                                                                    | Proof                                                                                                                                                                                                                                        |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1. Test asserts even-length, leading byte <0x80, round-trip for counters incl. 1,127,128,255,256,4095,4096,32768; fails red against current `padStart` first | **proven** — `api/test/helpers/tls-origin.test.ts:61 "encodes as a positive integer"` and `:73 "survives forge's DER writer"`; red run reproduced independently: 14/23 fail against the reverted body, restored and re-confirmed 23/23 green |
...
| 4. Comment at `tls-origin.ts@03f7fa4:143-149` "cert.publicKey = keys.publicKey" unchanged                                                                    | **proven** — text identical `1fe5a4d` vs `dab661c`. Ticket's own citation is off by one at the start (143 is `cert.publicKey = keys.publicKey;`; the comment itself is 144-149) — not spent as a finding, per instruction                    |

- **findings** · self-run hunt at medium returned 0; 0 carried, 0 dropped.
- NFR: security n/a (test-only fixture; correctness-improving) · performance n/a (new spec ~0.5s) · reliability + (removes a latent defect class) · maintainability + (extraction is testable, precedent-consistent with `tls-interception.ts@e9f516b:121-125` "function newSerial(): string {").
...
**3. Call-site wiring, verified past the unit level.** Static: `cert.serialNumber = fixtureSerialNumberHex(serialCounter);` at `api/test/helpers/tls-origin.ts:177 "cert.serialNumber = fixtureSerialNumberHex(serialCounter);"`. Dynamic: called `createFixtureCertificate` 128 times in sequence against the fixed tree, parsed the PEM with `node:crypto`'s `X509Certificate`: `serialNumber = "80"` (positive). Mutated back to the old body, repeated the same 128-call sequence: `serialNumber = "-80"` — matches the ticket's Log claim exactly.

**4. Counter coverage, enumerated not sampled.** `CASES` at `tls-origin.test.ts:28-38 "{ counter: 1, expected:"` has all 8 required counters (1, 127, 128, 255, 256, 4095, 4096, 32768) plus 3 extra. Every case runs both `test.each` blocks — `:61 "encodes as a positive integer"` asserts exact hex, even length, leading byte <0x80, round-trip; `:73 "survives forge's DER writer"` asserts the DER round-trip plus signed two's-complement read-back. All three requested properties present for every required counter.
