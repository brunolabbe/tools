...
| Done when                                                                                                                                                        | Proof                                                                                                                                                                                                                                                                                                                                                                          |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| A test drives a silent progressive variant through the engine into a container that forces a mux, and completes rather than failing on `-map 0:a`; fails on main | `api/test/silent-progressive-mux.test.ts:178-207 "downloads into a non-matching container instead of failing on -map 0:a"` ✓ — reproduced the pre-fix failure directly: copied the test onto `origin/main`, ran it, got `AppError` with `code: 'MUX_FAILED', details: { exitCode: 234, stderr: "Stream map '0:a:0' matches no streams..." }`                                   |
| A test asserts the direct resolver does not report a verified-sounding `hasAudio` for a response that never mentioned audio                                      | `api/test/silent-progressive-mux.test.ts:170 "expect(found.hasAudio).toBeUndefined();"` ✓                                                                                                                                                                                                                                                                                      |
| A component test asserts a rendition with no width/height still renders a name in Quality, not `—`                                                               | `web/test/variant-table.test.tsx:163-177 "a variant with no resolution is named by its label"` ✓ — empty-label guard (`:184-193 "a variant with neither a resolution nor a label"`) confirmed to fire by mutation                                                                                                                                                              |
...
- **low** · `resolvers/test/browser/variants.test.ts:37 "in (built ?? {})).toBe(false)"` asserts `"hasAudio" in built === false` (key absence), which is stricter than the contract requires: `exactOptionalPropertyTypes` plus `hasAudio?: boolean | undefined` permits an explicit `hasAudio: undefined` as well as omission, no read-site in the codebase checks enumerability rather than value, and JSON erases the distinction at the wire boundary anyway. Recommend loosening to `toBeUndefined()`; not blocking.
...
- **resolved, self-found** · a third, more severe gap the builder found applying the same method to its own diff: `engine/src/index.ts`'s manifest arm (`hasAudio: variant.hasAudio !== false`, passed into HLS/DASH downloads) was also changed-but-untested, and unlike the other two it fails silently — reverting it to `=== true` does not throw; it downloads video only and exits 0. Now covered by `engine/test/hls-e2e.test.ts:333-356 "an unverified audio claim keeps the track rather than dropping it"`, which reads the produced file's stream table rather than checking arguments or exit status. I reproduced this one directly: `AssertionError: expected [ 'video' ] to include 'audio'` at `hls-e2e.test.ts@53b57ad:343-344` "const streams = await streamKinds(outcome.path)", with the download call itself resolving rather than rejecting.
...
- **2026-09-05 — gate round, and the coverage gap it found.** The gate returned
  ...
  **Two different mechanisms, not one.** `estimate.ts` and `index.ts`'s manifest
  ...
  `audioUrl`, while every producer that sets `audioUrl` (`manifest/hls.ts:494`,
  `manifest/dash.ts:433`, `ytdlp.ts:641`) writes a boolean. They are defensive, and their
...
tools/downloader/api/package.json` is empty, `@downloader/resolvers` was
  already at `tools/downloader/api/package.json:19` under `dependencies`, and the `Dockerfile`
