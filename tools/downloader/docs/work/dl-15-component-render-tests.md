...
| Step 4 — `sourceUrlSchema` rejection surfacing, not swallowed         | `url-form.test.tsx:119 "a scheme the contract refuses still reaches the parent"` (reaches the parent), `app.test.tsx:274 "an address the contract refuses is rejected here, without a request"` (becomes an `INVALID_URL` panel)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| Step 4 — `App`'s `USING_MOCK_API` banner                              | `app.test.tsx:130 "a mocked build admits it, and offers the scenarios that reach every branch"` (present when mocked), `app.test.tsx:140 "a real build carries no mention of a mock anywhere on the page"` (absent when not)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Step 4 — `App`'s `probeToken` race                                    | **Both arms, since gate 4.** Resolve: `app.test.tsx:153 "a late response from an abandoned analysis never overwrites the current one"`, and the abandon path at `:250 "abandoning the wait also abandons the response that follows it"`. Reject — the likelier arm, and unproven until gate 4: `app.test.tsx:188 "from an abandoned analysis is dropped too"`, and the abandon path at `:227 "a rejection that arrives after the wait was abandoned raises nothing"`. Deleting either guard alone now reddens 2                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| 9   | low | `App`'s `startError`/`retryJob` shell paths uncovered; the jsdom native-validation assertion is environment-dependent                                                                                  | **accepted, and unmarked in the code** — the only trace of the uncovered paths is the string "not exercised by this suite" in `app.test.tsx`'s fake client, which names neither. The jsdom assertion _is_ commented, at `url-form.test.tsx:137 "the field's own type refuses a string that is not a URL at all"`. Recorded here instead |
...
| C   | med  | The probe race half-proven: both race tests only ever _resolved_. Deleting the `catch`-side `probeToken` guard alone left 162 green — and a slow probe is likelier to reject than to succeed late                                                                                                     | **fixed** — `app.test.tsx:188 "from an abandoned analysis is dropped too"` and `:227 "a rejection that arrives after the wait was abandoned raises nothing"`; the catch-side mutant reddens 2. Log sentence and acceptance row both corrected |
