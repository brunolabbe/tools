...
| Done when                                                                                                                                                       | Proof                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Migration test takes a fresh database and one at migration 4 to migration 5; both have the table and `jobs.host`                                                | `tools/downloader/api/test/schema.test.ts:27 "gets probe_outcomes and jobs.host from one migrate() pass"` ✓ · `tools/downloader/api/test/schema.test.ts:50 "gets the same table and column from the remaining migrate() pass"` ✓. Mutation (ALTER removed) turns 66/66 red. Also verified by hand on a migration-4 file database with 4 `jobs` rows: `user_version` 4→5, every old row `host` NULL; the report does not read `jobs.host`                                                                                                                                                                                                                                                                                                                                                                                                                                |
| One `probe_outcomes` row each for success, failure (each attempt's resolver, code, duration), cache hit, gate refusal; none for a per-client rate-limit refusal | success `tools/downloader/api/test/probe-outcomes.test.ts:53 "toBeGreaterThanOrEqual(0)"` ✓ · failure `tools/downloader/api/test/probe-outcomes.test.ts:89 "expect(rows[0]?.attempts).toEqual(["` ✓ · cache `tools/downloader/api/test/probe-outcomes.test.ts:115 "cached: true, durationMs: 0"` ✓ · gate `tools/downloader/api/test/probe-outcomes.test.ts:160 "const refusal = rows.find((row) =>"` ✓ · none per-client the per-client rate-limit refusal test in `tools/downloader/api/test/probe-outcomes.test.ts` at the reviewed commit `790c17b` (lines 163-169), titled "a per-client rate-limit refusal records nothing" ✓. Each recording site mutated away alone turns its own spec red, and so does adding a row in the bucket hook. Failure clause holds for a terminal error; a deadline/cancel failure drops the tier that ran out the clock — med below |
| Path + `?sig=` never in `probe_outcomes` or `jobs.host`, nor the client address                                                                                 | `tools/downloader/api/test/probe-outcomes.test.ts:207 "const serialized = JSON.stringify(row);"` ✓ · `tools/downloader/api/test/probe-outcomes.test.ts:259 "harness.app.context.store.jobHost(jobId)"` ✓ · client address structurally, no column can hold it: `tools/downloader/api/test/job-store.test.ts:494 "expect(Object.keys(row as object).toSorted()).toEqual("` ✓ (low below). `host = url.href` → 3 red; `jobs.host` = source URL → 3 red                                                                                                                                                                                                                                                                                                                                                                                                                    |
| A failing outcome write leaves the probe's response unchanged                                                                                                   | `tools/downloader/api/test/probe-outcomes.test.ts:236 "expect((response.json() as { cached: boolean }).cached).toBe(false)"` ✓. Wrapper made to rethrow → that spec red                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| Retention test with a controlled clock prunes rows older than `OUTCOME_RETENTION_DAYS` and keeps newer ones                                                     | **unproven** — `tools/downloader/api/test/job-store.test.ts:510 "pruneProbeOutcomes drops rows older than the cutoff"` passes a literal cutoff; no clock, no setting. The arithmetic at `tools/downloader/api/src/server.ts@20c8fd1:730-735 "const outcomesPruned ="` survives days→hours, never-prune and an inverted zero guard (0 red each)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- **high** · the pre-fix run command in `docs/02-DEPLOYMENT.md` at the reviewed commit `790c17b` (line 491), reading `docker compose exec downloader node dist/report.js --days 7` and the pre-fix run command in `tools/downloader/api/src/report.ts`'s module doc-comment at the reviewed commit `790c17b` (line 10), reading `docker compose exec downloader node dist/report.js --days 7` (copied from the brief's Build 7 and its last Done-when line) name a path that does not exist in the image. The runtime stage's working directory is /app and the api dist lands at `tools/downloader/Dockerfile:152 "tools/downloader/api/dist/main.js"`'s directory, so `node dist/report.js` resolves to `/app/dist/report.js`. Reproduced from the repo root (the /app layout): MODULE_NOT_FOUND, exit 1; `node tools/downloader/api/dist/report.js --days 1` prints the report, exit 0. No Docker daemon here, so not run in a container. An operator following the doc gets a stack trace. Fix: full path (recommended, matches CMD), or `exec -w /app/tools/downloader/api`.
...
- **med** · `tools/downloader/resolvers/src/registry.ts:128 "const error = AppError.from(cause);"` — `abortIfNeeded` throws before the attempt is pushed, so the tier the deadline or a cancel cut off never enters `attempts`. Reproduced: a `direct` miss then a hanging `browser` with `timeoutMs: 150` throws TIMEOUT with attempts `[{direct, NO_MEDIA_FOUND, 0}]`; a caller abort gives the same. A TIMEOUT row, and the report's "tried …" list for that host, omit the expensive tier the ticket's Why wants timed.
- **low** · the pre-fix percentile index in `tools/downloader/api/src/report.ts`'s `percentile` function at the reviewed commit `790c17b` (line 128), reading `Math.floor(p * sorted.length)` is upper-rank: p50 of [100,200,300,400] prints 300, and p50 of two downloads is the larger. Pin the definition in the report test.
- **low** · the pre-fix direct-invocation check in `tools/downloader/api/src/report.ts` at the reviewed commit `790c17b` (line 333), reading `if (import.meta.url === ` compares against a hand-built `file://` string. Run through a symlink, the report prints nothing and exits 0. Not live at the image path.
...
- **open decision** (to the orchestrator) · guard-stage exits get no row: INVALID_URL, BLOCKED_TARGET, UNREACHABLE (`tools/downloader/api/src/ssrf.ts:338 "That address could not be resolved."`), all thrown before `host` exists at the guard call in `tools/downloader/api/src/routes/probe.ts` at the reviewed commit `790c17b` (line 50), reading `const url = await context.guard.assertAllowed(rawUrl);`. Build 4 says "every way out" and "an AppError failure"; the Why asks "which sites fail". The builder's "no host" holds only for an unparseable URL.
- **open decision** (to the orchestrator) · a client that navigates away aborts the probe (`tools/downloader/api/src/routes/probe.ts@20eb8ba:115 "controller.abort(new AppError("`), recorded as CANCELED and counted as a failure in the probe success rate and in "hosts that fail most". dl-53 owns disconnect semantics for downloads; nothing names it for probes.
...
| One `probe_outcomes` row each for success, failure (each attempt's resolver, code, duration), cache hit, gate refusal; none for a per-client rate-limit refusal | success `tools/downloader/api/test/probe-outcomes.test.ts:53 "toBeGreaterThanOrEqual(0)"` ✓ · failure `tools/downloader/api/test/probe-outcomes.test.ts:89 "expect(rows[0]?.attempts).toEqual(["` ✓ · cache `tools/downloader/api/test/probe-outcomes.test.ts:115 "cached: true, durationMs: 0"` ✓ · gate `tools/downloader/api/test/probe-outcomes.test.ts:160 "const refusal = rows.find((row) =>"` ✓ · none per-client the per-client rate-limit refusal test in `tools/downloader/api/test/probe-outcomes.test.ts` at the reviewed commit `7b0cdcb` (line 178), titled "a per-client rate-limit refusal records nothing" ✓. A tier cut off by the deadline or a cancel is now named: `tools/downloader/resolvers/test/registry.test.ts:259 "the tier the deadline cuts off is still named"` ✓ · `tools/downloader/resolvers/test/registry.test.ts:281 "the tier a caller cancel cuts off is named with CANCELED"` ✓; removing the abort-branch push turns both red |
| Path + `?sig=` never in `probe_outcomes` or `jobs.host`, nor the client address                                                                                 | `tools/downloader/api/test/probe-outcomes.test.ts:207 "const serialized = JSON.stringify(row);"` ✓ · client address now asserted: `tools/downloader/api/test/probe-outcomes.test.ts:211 "default remote address — `host` is computed from the"` ✓ and `tools/downloader/api/test/probe-outcomes.test.ts:266 "expect(host).not.toBe("` ✓. `host = request.ip` turns the signed-URL spec red                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| A failing outcome write leaves the probe's response unchanged                                                                                                   | `tools/downloader/api/test/probe-outcomes.test.ts:236 "expect((response.json() as { cached: boolean }).cached).toBe(false)"` ✓ — unchanged since round 1                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Retention test with a controlled clock prunes rows older than `OUTCOME_RETENTION_DAYS` and keeps newer ones                                                     | `tools/downloader/api/test/probe-outcomes.test.ts:274 "a controlled clock prunes rows older than OUTCOME_RETENTION_DAYS"` ✓ · `tools/downloader/api/test/probe-outcomes.test.ts:295 "0 disables pruning entirely"` ✓. Days→hours, never-prune and the inverted zero guard each turn a spec red (1, 1, 2)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| Report test against a seeded database prints the expected rates and percentiles                                                                                 | `tools/downloader/api/test/report.test.ts:227 "expect(report.probes.total).toBe(9)"` ✓ · `tools/downloader/api/test/report.test.ts:253 "cache hits excluded, nearest-rank"` ✓ · `tools/downloader/api/test/report.test.ts:276 "expect(report.downloads.p50DurationMs).toBe(60_000)"` ✓ · probe window `tools/downloader/api/test/report.test.ts:282 "expect(withoutFilter?.successes).toBe(5)"` ✓ · text `tools/downloader/api/test/report.test.ts:302 "success rate: 75.0% (6/8 attempted"` ✓. Eight report mutations red (window with its parameter kept, percentile rank, gate string, cache exclusion, gate-in-failing-hosts, rate precision); the job-side window is not pinned — low below                                                                                                                                                                                                                                                                       |
...
- **resolved from round 1** · the high (report path), all three med (retention, report test, attempts on abort), and three low findings (percentile now nearest-rank at `tools/downloader/api/src/report.ts:148 "const rank = Math.max(1, Math.ceil(p * sorted.length));"`, direct-invocation check at `tools/downloader/api/src/report.ts:368 "function isMainModule(): boolean"` now prints through a symlink, the client-address clause now asserted). Each is re-run above, not taken from the builder's report.
...
- **low** · two findings, one mechanism: the report's job window at `tools/downloader/api/src/report.ts:234 "WHERE finished_at IS NOT NULL AND finished_at >= ?"` is unpinned. Ignoring the window with its parameter kept, or admitting unfinished jobs, leaves all 104 specs green, because every seeded job is finished and inside the window.
...
| Decision A: UNREACHABLE and BLOCKED_TARGET get a row; unparseable input none           | `tools/downloader/api/src/routes/probe.ts@20eb8ba:66 "const guardHost = hostnameOrNull(rawUrl);"` · `tools/downloader/api/test/probe-outcomes.test.ts:372 "BLOCKED_TARGET records a row, host masked as an IP literal"` ✓ · `tools/downloader/api/test/probe-outcomes.test.ts:402 "UNREACHABLE records a row with the guard"` ✓ (live DNS — low below) · `tools/downloader/api/test/probe-outcomes.test.ts:431 "an unparseable URL never reaches the guard"` ✓. Dropping the row: 2 red; dropping either code: 1 red each                                                                                                                                                                                                                                                                                                                                             |
...
- 2026-09-16 — Rebased onto `main` after dl-51 (#248, `01d66fa`), dl-55
  (#251, `1806525`) and dl-63 (#256, `2dc8365`) squash-merged, orphaning the
  branch's old base commits and the two gate-record pins (`790c17b`,
  `7b0cdcb`) built against them. `git checkout --detach
origin/dl-57-outcome-record && git rebase --onto origin/main 2e8aa7b`
  replayed the 7 commits unique to this branch with **no manual conflict**:
  the one conflict the rebase brief predicted — this branch's own re-pin of
  `tools/downloader/api/src/config.ts@95c6403:215` in dl-51's record colliding
  with dl-51's own re-pin of the same line, made independently before it
  merged — resolved itself, because both sides had written the identical
  text (`config.ts@95c6403:215`) and a three-way merge does not conflict on
  agreement. New tip `9dfe385` → `7c6ad76`.
- 2026-09-16 — Repaired this ticket's own Review citations, orphaned by the
  rebase. `node scripts/citations.mjs` found 40 machine-readable citations
  pinned `@790c17b` or `@7b0cdcb` (21 + 19); a 41st,
  `tools/downloader/Dockerfile@790c17b:152`, carries the same broken pin but
  is invisible to that script — `Dockerfile` has no extension the citation
  grammar recognises — and was found and fixed by hand. Of the 41: **34
  repointed** — the anchor still occurs exactly once in the cited file at
  this tip, so the `@sha` was dropped and the line number corrected to where
  the anchor now starts (checked with `citations.mjs`'s own `extractCitations`
  /`checkCitations`, unpinned, against the working tree, one citation per
  line so an anchor shared by two rows never borrows the other's line) — and
  **7 rewritten as prose**, naming the reviewed commit rather than pinning to
  it, because the anchor either occurs nowhere in the current file (5: the
  two `docker compose exec … node dist/report.js` pre-fix run commands in
  `docs/02-DEPLOYMENT.md` and `report.ts`'s doc-comment, the pre-fix
  percentile index, the pre-fix symlink-unsafe direct-invocation check, and
  the guard call in `routes/probe.ts` since restructured under decision A) or
  occurs twice (2: `expect(refused.statusCode).toBe(429)`, cited once each in
  round 1 and round 2 for "a per-client rate-limit refusal records nothing",
  now indistinguishable from an unrelated dl-51/dl-57-agreement assertion
  added in phase 2 that happens to read the same). No finding, verdict or
  acceptance line changed — only how each citation points at the commit the
  gate actually reviewed. Verified: `node scripts/citations.mjs
tools/downloader/docs/work/dl-57-a-record-of-how-probes-and-downloads-end.md
--section Review --require-anchors --require-distinct-anchors` — exit 0, 56
  verified, 0 moved, 0 unanchored, 0 unresolvable, 7 unchecked (the 7 prose
  citations, exactly), 1 pinned (the `@95c6403` one dl-51's own record owns).
  **Also checked, per the rebase brief's step 4**: this branch's rewrite of
  `routes/probe.ts`, `server.ts`, `registry.ts` and `config.ts` shifts lines
  that dl-19, dl-32, dl-34, dl-43, dl-44, dl-45, dl-46, dl-51, dl-55, dl-60
  and repo-33 (among others) cite unpinned in their own merged records —
  `node scripts/citations-gate.mjs --against origin/main` reports 81
  enforced, 0 failing, 7 grandfathered (none of them touching these files),
  **0 raised** against `origin/main`, and spot-checking dl-51, dl-55 and
  dl-60 individually with `citations.mjs --section Review
--require-anchors --require-distinct-anchors` each exits 0. Nothing moved
  out of range, so no new pin was needed on any other ticket's record.
  Verified after both repairs: `npm run build`; `npm run check` exit 0;
  `npm test -- --project downloader` — 82 files, 1386 tests, all green.
