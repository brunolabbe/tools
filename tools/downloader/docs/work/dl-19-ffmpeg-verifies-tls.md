...
1. **`-tls_verify` does not reach HLS/DASH segment fetches.** Proved by
   ...
   (`tools/downloader/docs/01-ARCHITECTURE.md@da81902:168` "TLS verification, and exactly how far it reaches.", `.env.example`, and this Log's opening);
...
3. **The boot warning (`tools/downloader/api/src/server.ts@da81902:103` "if (config.ffmpegAllowUnverifiedTls) {") has no test.** — **Not fixed**,
...
- **2026-09-13 — repo-44: repo-39's 3 pinned.** The boot-warning guard, twice, and the corrected architecture bullet are pinned to `da81902`, the commit that merged this record, where the warning stands as reviewed and the bullet reads as corrected.
