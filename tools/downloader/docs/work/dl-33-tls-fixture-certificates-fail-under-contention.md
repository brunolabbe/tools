...
table after the record resolves each one forward. (repo-44, 2026-09-13: `b48caf7` did not survive the squash merge, so these are now pinned per citation — the `egress-proxy.ts` coordinates to `2ca6549`, `main` just before this merged, where they stand as the reviewer read them, and the rest to `c2ff413`, the merge, where the table below says they did not move.)
...
| 3. If a fix lands, the failure is reproduced first, red before green             | **proven** — `tls-interception.test.ts@c2ff413:242-243` "expect(() => tls.createSecureContext({ cert: leaf.cert" (leaf-level, production call site), `tls-interception.test.ts@c2ff413:260-264` ".repeat(16)))).toBe" (encoding boundaries); independently reproduced by the reviewer at both unit and integration level (below) |
...
- **Unit-level red/green reproduced independently**: 12 tests green at `b48caf7` (1.96–1.99s). Reverting `positiveDerIntegerHex`'s body to the pre-fix unconditional-`00` logic reproduces the red at `tls.createSecureContext` (tls-interception.test.ts@c2ff413:242 "expect(() => tls.createSecureContext({ cert: leaf.cert") with the exact OpenSSL error.
...
| --------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `egress-proxy.ts:624:23`                                                                            | the `new tls.TLSSocket(clientSocket, …)` the uncaught stack named | **`egress-proxy.ts@c2ff413:718` "const clientTls = new tls.TLSSocket(clientSocket, {"**             | the guard moved it below a pre-built `SecureContext`                                                                                                                                                                                 |
| `egress-proxy.ts@2ca6549:614-628` "secureConnect"                                                   | the `secureConnect` handler                                       | **`egress-proxy.ts@c2ff413:681-723` "The leaf is issued and loaded before the client is told"**     | same commit; the handler grew the guard                                                                                                                                                                                              |
| `tls-interception.test.ts@c2ff413:242-243` "expect(() => tls.createSecureContext({ cert: leaf.cert" | the `createSecureContext` assertion                               | **`tls-interception.test.ts@c2ff413:242` "expect(() => tls.createSecureContext({ cert: leaf.cert"** | one line, not two — `oxfmt` collapsed it before `b48caf7` was committed, so this was a half-line-stale citation rather than a moved one                                                                                              |
| `two-origin-tls.test.ts` `afterAll:131`                                                             | —                                                                 | **`two-origin-tls.test.ts:123 "afterAll(async () => {"`**                                           | **wrong at `b48caf7` too.** `afterAll` is at line 123, which is also what the ticket's own sighting-3 quote says (`Hook timed out … ❯ two-origin-tls.test.ts:123:1`). The finding it supports is unaffected; only the number was off |
| `tls-interception.ts@c2ff413:147-150` "export function positiveDerIntegerHex(bytes: string)"        | `positiveDerIntegerHex`                                           | `tls-interception.ts@c2ff413:147-151` "export function positiveDerIntegerHex(bytes: string)"        | unmoved; the closing brace is 151                                                                                                                                                                                                    |
| `test/helpers/tls-origin.ts:125-129 "export function fixtureSerialNumberHex"`                       | `fixtureSerialNumberHex`                                          | unchanged                                                                                           | unmoved                                                                                                                                                                                                                              |
| `tls-interception.test.ts@c2ff413:260-264` ".repeat(16)))).toBe"                                    | the encoding-boundary assertions                                  | unchanged                                                                                           | unmoved                                                                                                                                                                                                                              |
...
- Citations re-verified: `citations.mjs --rev b48caf7` gives 17/17 independently; the re-resolution table's current-tip line numbers (`egress-proxy.ts@c2ff413:718` "const clientTls = new tls.TLSSocket(clientSocket, {", `:681-723` "The leaf is issued and loaded before the client is told", `tls-interception.ts@c2ff413:147-151` "export function positiveDerIntegerHex(bytes: string)") spot-checked correct.
...
- **2026-09-12 — repo-39: 20 failing references down to 17, and the 17 are this record's own instruction.** The gate record says in its own words that the reviewer's line numbers are left as written, as the evidence for what it ran at `b48caf7`, and the table after it resolves them forward "at this tip". Both columns are dated measurements, and repointing either to today would destroy what the record says it keeps. The three citations whose lines are byte-identical today are anchored: the fixture-serial helper, twice, and the table's `afterAll` coordinate, which was a bare shorthand bound to `tls-interception.test.ts` and now names `two-origin-tls.test.ts`. **Everything else waits for repo-35's per-citation pin**, which is the mechanism this record's "check them with `--rev b48caf7`" was asking for. One of the 17 cannot take an anchor even then without altering what it quotes: a coordinate inside the Vitest output string the table's sighting-3 note reproduces. `GRANDFATHERED` 20 → 17.

- **2026-09-13 — repo-44: 17 failing references down to 3.** `b48caf7` did not survive the squash merge. The `egress-proxy.ts` coordinates are pinned to `2ca6549`, `main` just before this merged, where they stand as the reviewer read them; the test and `tls-interception.ts` coordinates, which the table says did not move, and the table's current-tip column are pinned to `c2ff413`, the merge. The addendum's line 151 had bound to the test file and names the table's `positiveDerIntegerHex` range. A dated note beside the sentence that dates the reviewer's numbers says so.

  **Three cannot be repaired**, and stay counted in `GRANDFATHERED` on the owner's decision of 2026-09-13: the Vitest output the table quotes, which was the owner's answer to repo-44's Build; and the stack frame at `egress-proxy.ts` line 624, column 23, quoted with its column twice — once in the integration-reproduction bullet and once in the table's cited column. An anchor must follow a line number directly, so a coordinate carrying a column cannot take one without deleting the column. repo-44's Build said there was one such reference here; there are three.
