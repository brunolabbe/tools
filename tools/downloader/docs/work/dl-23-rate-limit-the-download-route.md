...
| Done when                                                                                                                 | Proof                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| ------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1. `GET /api/files/:token` refuses with 429 + `Retry-After` once exhausted                                                | proven — `rate-limit.test.ts:472` "refuses with a 429 and a Retry-After once the bucket is empty"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 2. The measured 274 req/min interrupted-scrub burst passes at the shipped default                                         | proven — `rate-limit.test.ts:525` "a seeking player's measured burst passes at the shipped default", which asserts `MEASURED_SCRUB_BURST = 274 < API_DEFAULTS.rateLimitFilesPerMinute` and then drives 274 real `.inject()` calls, all 206                                                                                                                                                                                                                                                                                                                                                      |
| 3. `RATE_LIMIT_FILES_PER_MINUTE=0` disables the limit                                                                     | proven — `rate-limit.test.ts:551` "zero disables it, for an operator serving large files to few people"                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
| 4. Keyed as Build step 2 decided; a second token from the same client is unaffected by the first token's exhausted bucket | proven — `rate-limit.test.ts:572` "one exhausted link does not spend another link's allowance" (same simulated address, distinct tokens, second unaffected)                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
- **findings** · 3 identified at medium depth: 1 carried, 2 dropped with reasons
  ...
  log line (`rate-limit.test.ts:624 "the bucket key never contains the token itself"`).
...
- **2026-09-12 — repo-39: the `## Review` citations anchored, 5 failing references down to 0, and the `GRANDFATHERED` entry deleted.** All five `rate-limit.test.ts` pointers had drifted by exactly seven lines, onto neighbouring tests' bodies, and are repointed by name. Outside the gate record, the filing entry's `routes/files.ts` coordinate is the CodeQL alert's own location and is left as the alert gave it.
