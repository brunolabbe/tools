...
Consolidates gates run against three tips of this branch — `9839f6e`, `4c2a1ed`, `d3677ad` — as the branch was built, then narrowed by an owner decision to revert the yt-dlp tier's mirror grouping and file it separately (dl-47). Citations below are anchored to `d3677ad`, the tip that ships.

| Done when                                                                                                                                                                 | Proof                                                                                                                                             |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- |
| A test proves a master declaring one rung at two hosts yields one variant carrying the second host as an alternate, rather than two variants.                             | `resolvers/test/hls.test.ts:312-318` ✓                                                                                                            |
| A test proves the engine, on a connection-level failure from the first host, retries the alternate and succeeds — driven by a fixture origin, not a mocked retry counter. | `engine/test/mirror-failover.test.ts:171` ✓ — verified by running it: real `spawn`'d ffmpeg against a dead port, 11214ms                          |
| A test proves an expired signed URL does not trigger a mirror attempt: it re-probes, as it does today.                                                                    | `engine/test/mirror-failover.test.ts:206` + unit-level `engine/test/failover.test.ts:33` ✓                                                        |
| The picker still shows one row per rendition, and dl-40's fixtures still pass unchanged.                                                                                  | `web/test/variant-table.test.tsx:285` (one row per rung) + `web/test/presentation-helpers.test.ts:268,283` (`collapsed=5`, unchanged from base) ✓ |
...
- **SSRF coverage, traced end to end and red-checked, not just diffed.** `urlsInProbeResult` (`api/src/ssrf.ts`) pushes `alternateUrls` into `mustPass`; its only two callers are `api/src/routes/probe.ts:134` and `api/src/jobs/orchestrator.ts:220`, both of which run before the orchestrator's single `engine.download` call site (`api/src/jobs/orchestrator.ts:268`) — and the orchestrator never reads a probe cache (its own docblock), so every attempt re-guards a fresh probe. Red-checked by stubbing the `alternateUrls` loop to `[]` in `ssrf.ts`: the new test at `api/test/ssrf.test.ts:170` failed as expected (`mustPass` missing the 169.254.169.254 alternate), restored, clean. Redirect-after-guard is handled by pre-existing, undiffed infrastructure (`guarded-fetch.ts` re-checks every hop for the engine's own fetches; `egress-proxy.ts` vets every ffmpeg-subprocess connection at the socket level) that applies uniformly to a mirror because `engine/src/index.ts:396`'s `#downloadFrom` takes the URL as a plain parameter into identical download machinery regardless of primary or alternate.
- **Contract shape matches Build step 1 exactly**, no unilateral widening: `alternateUrls?: string[] | undefined` (`contract/src/media.ts:110`), `z.array(z.string().min(1)).optional()` schema, round-trip parse test at `contract/test/contract-schemas.test.ts:151` that would catch silent field-stripping (e.g. on the SQLite job-row round trip).
- **dl-40 objection answered and verified.** `displayKey` (`web/src/lib/variants.ts:159-173`) excludes `mirrors` by design; `web/test/presentation-helpers.test.ts:225` builds a same-attribute twin differing only in mirror count and asserts it still collapses to one row.
- **TLS_VERIFICATION_FAILED inclusion, an owner override of the builder's own recommendation, checked rather than trusted.** `manifest.ts:67-73`'s `tlsOptions` sources `tlsVerify`/`tlsCaFile` from unchanging engine config on every candidate, so a mirror's certificate is verified exactly as strictly as the primary's — the failover has no path to downgrade verification, since it never carries a verification setting of its own. The loop (`engine/src/index.ts:369`) throws whichever error the _current_ candidate raised, never a stored first error, so a wholly-MITM'd path still surfaces `TLS_VERIFICATION_FAILED` at the last candidate — proven at `engine/test/mirror-failover.test.ts:227` ("the last candidate's own failure is what reaches the caller"). Red-checked myself: removed `"TLS_VERIFICATION_FAILED"` from `HOST_FAILURE_CODES` (`failover.ts:86`), 2 of `failover.test.ts`'s tests failed (including `:94`), restored, clean.
- **Stderr classifier's unrecognised→false default is tested, not assumed.** `engine/test/failover.test.ts:116` proves an unrecognised failure string returns `false` from `isHostFailure` — no failover, falls through to the existing failure/reprobe path, the conservative direction the docblock in `failover.ts` argues for.
