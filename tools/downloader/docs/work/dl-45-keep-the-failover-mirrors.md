...
**Gate: PASS** — 2026-09-07 · `origin/main...HEAD`, tip `d3677ad` · own defect hunt (ticket-reviewer subagent, no `code-review` dispatch)

Consolidates gates run against three tips of this branch — `9839f6e`, `4c2a1ed`, `d3677ad` — as the branch was built, then narrowed by an owner decision to revert the yt-dlp tier's mirror grouping and file it separately (dl-47). Citations below are anchored to `d3677ad`, the tip that ships. (repo-39, 2026-09-12: four coordinates here that dl-47 later moved — three `mirror-failover.test.ts` tests and the mirror-count `presentation-helpers.test.ts` test — are repointed to where the same tests stand today, and the `failover.test.ts` shorthand in the TLS bullet is qualified because it had bound to `failover.ts`. The picker-collapse pair in row four names a test dl-47 replaced, and is left as written. repo-44, 2026-09-13: that pair is now pinned per citation to `1f0f440`, the commit that merged this record, where the replaced test still stands.)

| Done when                                                                                                                                                                 | Proof                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A test proves a master declaring one rung at two hosts yields one variant carrying the second host as an alternate, rather than two variants.                             | `resolvers/test/hls.test.ts:312-318 "one rung at two hosts is one variant carrying the second host as an alternate"` ✓                                                                                                                                                                                                                                |
| A test proves the engine, on a connection-level failure from the first host, retries the alternate and succeeds — driven by a fixture origin, not a mocked retry counter. | `engine/test/mirror-failover.test.ts:275 "a host that refuses the connection is failed over to the mirror"` ✓ — verified by running it: real `spawn`'d ffmpeg against a dead port, 11214ms                                                                                                                                                            |
| A test proves an expired signed URL does not trigger a mirror attempt: it re-probes, as it does today.                                                                    | `engine/test/mirror-failover.test.ts:310 "an expired signed URL never reaches the mirror"` + unit-level `engine/test/failover.test.ts:33 "an expired signed URL is not, whichever layer noticed"` ✓                                                                                                                                                   |
| The picker still shows one row per rendition, and dl-40's fixtures still pass unchanged.                                                                                  | `web/test/variant-table.test.tsx:285 "mirrors of a rendition render as one row per rung"` (one row per rung) + `web/test/presentation-helpers.test.ts@1f0f440:268` "the picker's collapse and the tier's dedup are different", `web/test/presentation-helpers.test.ts@1f0f440:283` "expect(collapsed).toBe(5)" (`collapsed=5`, unchanged from base) ✓ |
| `npm run check` and `npm test -- --project downloader` pass.                                                                                                              | verified — `npm run check` exit 0; `npm test -- --project downloader` 1184/1184 over 73 files, run at `d3677ad`                                                                                                                                                                                                                                       |

- **SSRF coverage, traced end to end and red-checked, not just diffed.** `urlsInProbeResult` (`api/src/ssrf.ts`) pushes `alternateUrls` into `mustPass`; its only two callers are `api/src/routes/probe.ts@95c6403:134 "assertAllAllowed(urlsInProbeResult(probe).mustPass)"` and `api/src/jobs/orchestrator.ts@20c8fd1:220 "assertAllAllowed(urlsInProbeResult(probe).mustPass)"`, both of which run before the orchestrator's single `engine.download` call site (`api/src/jobs/orchestrator.ts@20c8fd1:268 "const outcome = await engine.download({"`) — and the orchestrator never reads a probe cache (its own docblock), so every attempt re-guards a fresh probe. Red-checked by stubbing the `alternateUrls` loop to `[]` in `ssrf.ts`: the new test at `api/test/ssrf.test.ts:170 "a failover mirror is vetted like the address it stands in for"` failed as expected (`mustPass` missing the 169.254.169.254 alternate), restored, clean. Redirect-after-guard is handled by pre-existing, undiffed infrastructure (`guarded-fetch.ts` re-checks every hop for the engine's own fetches; `egress-proxy.ts` vets every ffmpeg-subprocess connection at the socket level) that applies uniformly to a mirror because `engine/src/index.ts:396 "async #downloadFrom("`'s `#downloadFrom` takes the URL as a plain parameter into identical download machinery regardless of primary or alternate.
- **Contract shape matches Build step 1 exactly**, no unilateral widening: `alternateUrls?: string[] | undefined` (`contract/src/media.ts:110 "alternateUrls?: string[] | undefined;"`), `z.array(z.string().min(1)).optional()` schema, round-trip parse test at `contract/test/contract-schemas.test.ts:151 "failover mirrors survive the parse, and an empty one does not"` that would catch silent field-stripping (e.g. on the SQLite job-row round trip).
- **dl-40 objection answered and verified.** `displayKey` (`web/src/lib/variants.ts:159-173 "function displayKey"`) excludes `mirrors` by design; `web/test/presentation-helpers.test.ts:224 "the mirror count reaches the row, unequal per rung, and is not a row key"` builds a same-attribute twin differing only in mirror count and asserts it still collapses to one row.
- **TLS_VERIFICATION_FAILED inclusion, an owner override of the builder's own recommendation, checked rather than trusted.** `manifest.ts:67-73 "function tlsOptions"`'s `tlsOptions` sources `tlsVerify`/`tlsCaFile` from unchanging engine config on every candidate, so a mirror's certificate is verified exactly as strictly as the primary's — the failover has no path to downgrade verification, since it never carries a verification setting of its own. The loop (`engine/src/index.ts:369 "if (!another || !isHostFailure(error)) throw error;"`) throws whichever error the _current_ candidate raised, never a stored first error, so a wholly-MITM'd path still surfaces `TLS_VERIFICATION_FAILED` at the last candidate — proven at `engine/test/mirror-failover.test.ts:331 "the last candidate's own failure is what reaches the caller"` ("the last candidate's own failure is what reaches the caller"). Red-checked myself: removed `"TLS_VERIFICATION_FAILED"` from `HOST_FAILURE_CODES` (`failover.ts:86 "const HOST_FAILURE_CODES"`), 2 of `failover.test.ts`'s tests failed (including `engine/test/failover.test.ts:94 "a rejected certificate buys a mirror, over a recorded objection"`), restored, clean.
- **Stderr classifier's unrecognised→false default is tested, not assumed.** `engine/test/failover.test.ts:116 "anything that is not an AppError, and anything unrecognised, is not"` proves an unrecognised failure string returns `false` from `isHostFailure` — no failover, falls through to the existing failure/reprobe path, the conservative direction the docblock in `failover.ts` argues for.
- **The 11s tests are real, not a lazy sleep.** `mirror-failover.test.ts` has no `sleep`/`setTimeout`; two of its three original cases cost ~11.2s each from a real `spawn`'d ffmpeg's own `-reconnect_delay_max 10` against a dead port, confirmed unchanged from `main` (`git diff origin/main -- engine/src | grep reconnect` empty).
- **A real finding was made during this review, is not present in this diff, and is why the branch narrowed.** During an interim tip (`4c2a1ed`) the yt-dlp tier's mirrors were grouped by the same `groupMirrors`/`renditionKey` the HLS parser uses. I reproduced, not hypothesised, that this can silently merge two genuinely different yt-dlp renditions (e.g. distinct-language audio tracks) into one variant when neither carries a `language` tag and their technical metadata coincidentally matches — verified directly against `mapYtDlpInfo` with an `audio-en`/`audio-fr` pair, both `acodec: "mp4a.40.2"`/`abr: 128`, neither tagged: the mapper returned one variant with the French URL demoted into `alternateUrls`. Consequence: the picker never offers the second track, and a host failure on the first would silently download the _other_ track's content under the label the user selected — a correctness defect, not a display gap. This was carried, not dropped: the owner reverted the fold-in in response and it is now `dl-47`, with two remedies recorded as an open decision rather than settled unilaterally by either of us. Verified the ticket's reproduction is faithful to what I actually ran. **Correction to my own initial framing, verified independently**: the _lost choice_ half of the harm is not new — `toDisplayRows` on the ungrouped pair already returns one row (`collapsed: 1`), so the picker's dl-40 collapse had already merged these before dl-45 existed; the defect dl-45's fold-in specifically added was the discarded URL becoming a live failover target.
- **Registration / invariants swept**: no shell invocation, argument arrays throughout; `redactUrl` used on both URLs at the mirror-failover log line (`engine/src/index.ts` `#downloadMedia`); no new `AppError` codes invented (`UNREACHABLE`/`DOWNLOAD_FAILED`/`VARIANT_GONE`/now `TLS_VERIFICATION_FAILED` in `HOST_FAILURE_CODES`, all pre-existing core codes); no faked progress (a mirror restart re-emits real ffmpeg progress against the same known duration); no `any`/bare `console`/missing `import type` in the diff; new tests run under `npm test -- --project downloader` without extra registration needed (existing project globs). Contract change shown being agreed rather than assumed, per the ticket's own Build step 1 and Log.
- **findings** · own hunt across all three tips returned 1; carried (led to the yt-dlp fold-in being reverted from this diff and filed as `dl-47` with the tradeoff open, rather than fixed in place or dropped); 0 dropped. Against the final diff (`d3677ad`) specifically: 0 new findings.
- NFR: security ✓ (SSRF coverage traced and red-checked; TLS-mirror path verified not to weaken cert verification) · performance — the real ~11.2s reconnect cost is production ffmpeg behaviour, unchanged by this branch, reflected honestly in test duration rather than mocked away · reliability ✓ (failover restarts cleanly and clears tmp; unrecognised-failure default is the conservative direction; last-candidate error propagates undegraded) · maintainability ✓ (dl-47 preserves the open tradeoff as a decision rather than resolving it quietly, consistent with this ticket's own pattern for the TLS and mirror-count decisions).

**Two coordinates were repointed by the builder before committing, and the reviewer was told.** Neither changes a claim; both were wrong in shape.

- The record as written cited `web/src/lib/variants.ts:159-186 "function displayKey"` for `displayKey`. Re-resolved against `d3677ad`, `displayKey` is lines 159–173; 175–184 is `groupByKey` and 186 is the `DisplayRows` docblock, so the range overshot the symbol it names. Corrected to `159-173`.
- It cited the `engine.download` call site by bare filename — `orchestrator` plus `.ts` plus the line, with no directory — having named the file in full earlier in the same bullet. Spelled out in words here rather than reproduced, because the script parses the broken form out of this paragraph too and fails on the example. `scripts/citations.mjs` reported that one **unresolvable** and exited 1: there are three `orchestrator.ts` in this repo — `downloader/api/src/jobs/`, `planner/agent/src/` and `planner/api/src/runs/` — so a bare filename names no file. `manifest.ts` and `failover.ts` in the bullets below survive the same shorthand only because each is unique in the tree, which is luck rather than a convention to copy. Qualified to `api/src/jobs/orchestrator.ts@20c8fd1:268 "const outcome = await engine.download({"`; line 268 reads `const outcome = await engine.download({`, so the claim was right and only the address was not.

Every other coordinate above was re-resolved line by line against the same tip and reads as the record says. That check is what this ticket's own Log entry of 2026-09-07 exists to demand, and it is not optional here: 34 of these 41 references carry no anchor text, so `scripts/citations.mjs` can confirm they point at a real line and nothing more. It cannot tell a correct line from a moved one without a quoted fragment — which is the same gap that entry recorded, still open, and now on a much larger record.

**Those two numbers first read 32 and 39, and the gate caught it.** They were copied from a run made _before_ this paragraph existed; the paragraph then added its own citations and moved the totals, and nobody re-ran the command. A count restated from an earlier run is a stale measurement wearing a measured number's clothes, and it appeared in the one sentence on this page whose entire subject is citation precision. Corrected after re-running, and the re-run was repeated after the correction itself — which is the actual lesson, since editing a file that counts its own references changes what it is counting.
