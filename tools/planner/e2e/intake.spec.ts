 ...
 *  - a real `fetch` round-trips an `Answer` through `answerSchema`, rather than
 *    a faked client agreeing with itself;
 ...
 *
 * The walk down the tree lives in `intake-walk.ts` since pl-19, because
 * `pin.spec.ts` needs a described trip before it has anything to pin. It moved
 * unchanged, and the rule it carries — never name a question — moved with it.
 */
