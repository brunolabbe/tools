/**
 * Getting to the checkpoint, without naming a question.
 *
 * Extracted from `intake.spec.ts` by pl-19, which needs the same walk to reach
 * something to draft. It is shared rather than copied because the rule it
 * encodes is the fragile part: **the tree is authored content and is reviewed as
 * content**, so a spec that types into `#field-road-trip.drive-appetite` or
 * counts eight questions turns a content edit into a red build. One copy of that
 * rule can be kept honest; two drift, and the second is the one nobody reads
 * before editing the tree.
 *
 ...
 */
export async function fillField(card: Locator): Promise<void> {
 ...
 */
export async function answerCurrent(page: Page): Promise<string> {
