...
| Done when                                                                                                        | Proof                                                                                                                                                                                                                                                         |
| ---------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Migration test: fresh DB and one at the previous migration both reach the new one with the columns               | `migrations.test.ts:149 "carry a column for each token kind"` and `migrations.test.ts:158 "reads as unrecorded"` ✓                                                                                                                                            |
| `usageOf` reports input, cache-read and cache-write apart, summed across `iterations`                            | `anthropic-provider.test.ts:256 "inputTokens: 700"` (apart) and `anthropic-provider.test.ts:288 "cacheReadTokens: 300"` (summed across iterations) ✓                                                                                                          |
| A `done`, a `failed` and a `canceled` run each record usage; the canceled one counts replies that finished first | `run-usage.test.ts:134 "records every reply, each token kind"`, `run-usage.test.ts:152 "records the replies it was billed for before it failed"`, `run-usage.test.ts:174 "records the replies that finished before the cancellation"` ✓                       |
| A failing usage write leaves the run's outcome unchanged                                                         | `run-usage.test.ts:206 "leaves the run's outcome as it was, and says so"` ✓                                                                                                                                                                                   |
...
- NFR: security n/a (no URL or header handling touched) · performance n/a · reliability ✓ (`recordUsage` wraps the write, warns, never fails the run — `run-usage.test.ts:206 "leaves the run's outcome as it was, and says so"`) · maintainability — the low finding above, closed.
