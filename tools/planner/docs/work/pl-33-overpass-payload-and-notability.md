...
- **low** · `docs/02-DEPLOYMENT.md` has a full walkthrough and an env-var table for `VALHALLA_URL`/`GEOCODER_URL` (Valhalla + Nominatim), but nothing for the new `overpass` service, `OVERPASS_URL`, or `GROUNDING_DISCOVERY_TIMEOUT_MS` — even though `compose.planner.yaml:116 "tile_urls:"`, `compose.planner.yaml:149 "PBF_URL:"` and `compose.planner.yaml:186 "OVERPASS_PLANET_URL:"` all point a reader at that same doc for `OSM_EXTRACT_URL`. A deployer following the doc as written would never learn discovery can be turned on.
