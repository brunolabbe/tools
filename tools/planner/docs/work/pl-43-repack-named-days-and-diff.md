...
| The `moved` rule is on `RevisionDiff`'s doc comment, the only `contract` change                                                                                       | `contract/src/plan.ts:657 "What counts as"` (verified — `git diff 95c6403 1a9ffff -- tools/planner/contract` is comment-only)                                                                                    |
