...
| Done when                                                                                                       | Proof                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| --------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A plan list and a plan detail render from real data, list not loading revisions                                 | `api/test/plan-view.test.ts@8849c14:185 "lists plans without loading their revisions"` (list omits `revisions`/`candidates`/`brief`), `api/test/plan-view.test.ts@8849c14:104 "comes back with the document and what nothing checked about it"` (detail) ✓                                                                                                                                                                                                                                                              |
| Open-dates brief renders correctly, no invented dates                                                           | `web/test/plan-view.test.tsx@95c6403:75 "a dateless day renders by its index, and invents no date"` ✓                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Every item shows grounded/model-asserted; cost provenance divergence rendered honestly                          | `web/test/plan-view.test.tsx@95c6403:143 "a verified place with a guessed price says both"` (grounded place + guessed cost), `web/test/plan-view.test.tsx@95c6403:172 "an ungrounded candidate says so plainly"` (ungrounded) ✓                                                                                                                                                                                                                                                                                         |
| A `PlanGap` shows in the plan body, distinct sentence per `PlanGapReason`                                       | `web/test/plan-view.test.tsx@95c6403:297 "a gap shows its own sentence, in the plan body"`, `web/test/plan-view.test.tsx@95c6403:324 "distinct reasons read as distinct sentences"` — 3 of 4 reasons exercised directly; the 4th (`specialist-dropped-for-budget`) is covered only by `GAP_LABELS`'s TS-exhaustive `Record`, not a render assertion ✓                                                                                                                                                                   |
| Every plan says travel time was not checked, asserted on a plan loaded from the database, not one just composed | `api/test/plan-view.test.ts@8849c14:127 "says travel time was not checked, on a plan read back from the database"` — real SQLite, over HTTP, after `runToCompletion`, read twice ✓ (strong; exactly the trap named)                                                                                                                                                                                                                                                                                                     |
| Pinning from the UI persists and creates no revision, asserted                                                  | **proven, as of [pl-19](./pl-19-pin-through-the-browser.md)** — `e2e/pin.spec.ts` pins through a real browser against the real API, reloads, and finds the pin still there and the version line unchanged. Was `unproven (gate)` on this ticket's own gate, and the reason it stayed that way is in the note below.                                                                                                                                                                                                     |
| No cost is displayed as a single figure anywhere                                                                | **unproven (gate)** — `tools/planner/web/src/plan/format.ts:68-74 "export function describeCost"` renders `low === high` as `"20 EUR, a posted price"`, a bare single figure with a label. No test anywhere exercises this branch: the one cost test uses `low:40/high:60` (`web/test/plan-view.test.tsx@95c6403:217 "a cost renders as a band and never as one figure"`), and the default candidate fixture has `cost: null`. The interpretation is defensible but untested and, read literally, contradicts the line. |
| `web/test/tsconfig.json` exists, referenced from root, glob excludes exactly that path                          | Verified independently, not from the Log: file created in pl-12 (`7dde7ba`), `tsconfig.json:29 "./tools/planner/web/test"` references it, `tsconfig.tests.json`'s `exclude` names exactly `tools/downloader/web/test/**` and `tools/planner/web/test/**`. Untouched by this diff (`git diff` on the three files is empty) ✓                                                                                                                                                                                             |
...
- **med** · `describeCost` bends "no cost as a single figure": a fixed price (`low === high`) prints as one number, and the acceptance line's only interesting branch has zero test coverage. `web/src/plan/format.ts:68-74 "export function describeCost"`.
- **med** · The "from the UI" half of the pinning acceptance line is not actually proven from the UI — the web test mocks the API client, so persistence/no-revision is only checked at the HTTP layer. `web/test/plan-view.test.tsx@95c6403:513-540 "pins through the client and renders what comes back"` vs `api/test/plan-view.test.ts:225-256 "persists, and appends no revision"`.
- **low** · `uncheckedFor`'s currency detection silently changed behavior: the removed code looked placed ids up in `season.kept`, dropping a pinned-out-of-season candidate's currency; the new code looks the same ids up in the full candidate list, so it now counts. Very likely more correct, but unacknowledged in the Log and untested (no `compose.test.ts` case combines `previous`/pinned items with mixed currencies). Unreachable today — nothing calls `compose()` with `previous` outside tests, since re-plan is Phase 4. `tools/planner/itinerary/src/unchecked.ts:358-362 "const currencies = new Set("`.
- **low** · `updateItemPin` scopes only by `plan_id`, not by latest revision, so a pin aimed at an item on a superseded revision succeeds silently while `pinnedPlacements` only ever reads pins off the revision passed as `previous` — a write that looks successful but has no effect once Phase 4 exists. Unreachable today (no revise flow, no multi-revision plan in production). `tools/planner/api/src/db/plans.ts:275-290 "export function updateItemPin"`.
...
- **"Pinning from the UI" stays `unproven (gate)`, and that is the honest
  ...
  **Taken on 2026-08-21 as [pl-19](./pl-19-pin-through-the-browser.md), and the
  row above is now `proven`.** The scope call was made deliberately and
  separately, which is what this paragraph asked for. It was worth it: a
  mutation that pins in React state and never reaches the server passes the web
  test unchanged — the mock is the client module — and is caught only after the
  reload. The seam this row described was real.
