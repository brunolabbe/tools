...
| Open-dates brief renders correctly, no invented dates                                                           | `web/test/plan-view.test.tsx:75 "a dateless day renders by its index, and invents no date"` ✓                                                                                                                                                                                                                                                                                                                                                                                                                   |
| Every item shows grounded/model-asserted; cost provenance divergence rendered honestly                          | `web/test/plan-view.test.tsx:143 "a verified place with a guessed price says both"` (grounded place + guessed cost), `web/test/plan-view.test.tsx:172 "an ungrounded candidate says so plainly"` (ungrounded) ✓                                                                                                                                                                                                                                                                                                 |
| A `PlanGap` shows in the plan body, distinct sentence per `PlanGapReason`                                       | `web/test/plan-view.test.tsx:297 "a gap shows its own sentence, in the plan body"`, `web/test/plan-view.test.tsx:324 "distinct reasons read as distinct sentences"` — 3 of 4 reasons exercised directly; the 4th (`specialist-dropped-for-budget`) is covered only by `GAP_LABELS`'s TS-exhaustive `Record`, not a render assertion ✓                                                                                                                                                                           |
...
- **med** · The "from the UI" half of the pinning acceptance line is not actually proven from the UI — the web test mocks the API client, so persistence/no-revision is only checked at the HTTP layer. `web/test/plan-view.test.tsx:513-540 "pins through the client and renders what comes back"` vs `api/test/plan-view.test.ts:225-256 "persists, and appends no revision"`.
