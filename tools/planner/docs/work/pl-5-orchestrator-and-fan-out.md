...
| Done when                                                                                                                                                          | Proof                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rosterFor` table-driven, tested per shape incl. one deliberately absent                                                                                           | `agent/test/roster.test.ts:20 "the roster, per trip shape"` (per-shape), `agent/test/roster.test.ts:86 "const absent = decision.notApplicable.find"` (food absent, backcountry) ✓                                                                                                                                                                                                                                                                                                                                       |
| Every route candidate returned is `between`, per checked-in brief                                                                                                  | `agent/test/placeable.test.ts:113 "a route specialist that ran returned legs with both ends"` ✓                                                                                                                                                                                                                                                                                                                                                                                                                         |
| Run over checked-in briefs: candidates from every rostered specialist, per-specialist progress, survives one specialist failing with gap named, nothing fabricated | `agent/test/fan-out.test.ts:58 "produces candidates from every rostered specialist"` (candidates), `agent/test/fan-out.test.ts:138 "reports the roster before the fan-out, and then one event per specialist"` (progress), `agent/test/fan-out.test.ts:158 "the run ships and the plan says that part was not checked"` and `agent/test/fan-out.test.ts:175 "nothing is invented in its place"` (failure → gap, no fabrication) ✓ — wire-level SSE progress is pl-16's, landed on `main` (`09bd161`), outside this diff |
| Budget path tested: over-cap roster degraded before fan-out, drop recorded                                                                                         | `agent/test/budget.test.ts:26 "a roster over the cap is cut from the back of the order"`, `agent/test/budget.test.ts:47 "the architecture's default of five drops one from a six-specialist trip"`, `agent/test/budget.test.ts:74 "every specialist that will not contribute has a gap"`; `agent/test/fan-out.test.ts:259 "a roster over the cap is degraded before anything is sent"` ✓                                                                                                                                |
| Nothing in this ticket packs a day or writes a schedule                                                                                                            | `agent/test/fan-out.test.ts:128 "nothing it returns says which day anything falls on"` ✓                                                                                                                                                                                                                                                                                                                                                                                                                                |
| Candidates placeable: `compose()` over fan-out output places ≥1 candidate from every rostered schedulable specialist, or names the drop                            | `agent/test/placeable.test.ts:78 "places at least one candidate from every rostered schedulable specialist"` ✓                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- **med** · The close-out entry contradicted itself: "Every `Done when` line on this ticket was already met by the 2026-08-16 entry" was followed three sentences later by "What that entry could not claim — that a run streams per-specialist progress over a wire — is pl-16's." (repo-37: the three coordinates this bullet carried all named this record's own lines, and a record cannot cite itself under `--require-distinct-anchors` — the anchor text is written into the very file it points at, so it always occurs at least twice. They are named as sections instead: the paragraph was rewritten in answer to this finding, the corrected opening is the **All but one of the `Done when` lines** paragraph in the 2026-08-18 close-out, and the entry titled _the close-out corrected after its own gate_ records the correction.) The third `Done when` line above requires streaming, and the entry it cites explicitly says that was _not_ built by pl-5. The underlying substance is fine (pl-16 landed it, verified on `main`), but a reader taking the opening sentence at face value believes pl-5 met its own acceptance unaided. Two finders converged on this independently.
- **med** · `tools/planner/docs/02-ROADMAP.md:241-260 "Whether a specialist streams"`'s "Still open" list had broken nesting — the replacement paragraph is indented two spaces with no blank line and no `- ` marker, so CommonMark treats it as a lazy continuation of the preceding bullet ("Whether a specialist streams"). Rendered, the resolved `RunStatus`/`RunProgress`/`MAX_SPECIALISTS` answers appear to belong to that bullet, making the one item that is genuinely still open (streaming) read as answered. Three finders independently confirmed this by rendering it. Fix is a `- ` prefix or a blank line before the paragraph.
