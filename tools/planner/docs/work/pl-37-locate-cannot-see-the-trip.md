...
| Done when                                                                                                    | Proof                                                                                                                                                                                                                                                                                                                                                                    |
| ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Bare `Percé` + Québec destination locates the town, and still declines with no destination                   | `grounding-valhalla.test.ts:1020 "a bare name a locality never narrowed is located by the trip"` (locates) ✓ · `grounding-valhalla.test.ts:1032 "and the same reply with no destination still declines"` (declines) ✓                                                                                                                                                    |
| `locateKey` distinguishes two runs whose destinations differ, by a test that fails if the context is dropped | `grounding-cache.test.ts:876 "two trips to different destinations get two answers"` / `grounding-cache.test.ts:913 "a trip with no destination is a different question"` / `grounding-cache.test.ts:926 "the key itself distinguishes them"` ✓ — reproduced: dropping `trip` from `locateKey` fails 3 of the 4 new cache tests, the positive one stays green as designed |
| pl-34's ten-capture block still passes, with any changed outcome named                                       | `grounding-valhalla.test.ts:885 "nine of the ten locality-supplied lookups locate"` (unmodified pl-34 test, still 9/10) ✓ · `grounding-valhalla.test.ts:914 "with a Québec destination the bare name joins the nine"` (new: with a Québec destination, 10/10, nine unmoved) ✓                                                                                            |
...
- **Cache key.** Intra-run consistency verified by reading, not assuming: `orchestrator.ts` computes `tripContextFor(brief)` once per run (`tools/planner/api/src/runs/orchestrator.ts@95c6403:362 "trip: tripContextFor(brief),"` — repo-37 corrected the filename here, which read `travel.ts`; the line number was right and the file was not) and `discovery.ts` computes it once per corridor call (`discovery.ts@95c6403:225 "const trip = tripContextFor(brief);"`) from the same immutable `brief`, so every `locate` call in one run shares one `trip` value — no path produces two different trip contexts for one run. Collision check: `locateKey` = `placeIdentity(place) + KEY_SEPARATOR + destination-part`; `KEY_SEPARATOR`/`ABSENT` are control characters stripped by `normalisePart` before joining, so no user-controlled string can forge a separator across the (name, locality, destination) boundary — no two distinct triples can collide.
- **pl-34's allowlist.** `SETTLEMENT_ADDRESS_TYPES` (`valhalla.ts:206 "const SETTLEMENT_ADDRESS_TYPES"`) unchanged, still `{city, town, village, municipality}`, still referenced exactly once (`valhalla.ts:1006 "SETTLEMENT_ADDRESS_TYPES.has(each.addressType)"`), still behind locality only — no second reference was added behind the destination rung.
