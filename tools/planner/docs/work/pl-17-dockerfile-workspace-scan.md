...
| Done when                                                                                                     | Proof                                                                                                                                                                                                                                                                                                                                                                                           |
| ------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Deleting either `itinerary` line from the planner's `Dockerfile` turns the suite red                          | `packages/core/test/image-closure.test.ts:304 "has its manifest copied before"` and `packages/core/test/image-closure.test.ts:318 "has its manifest and"` ✓ — verified by mutation: removed the build-stage manifest line and, separately, the runtime `dist` line; each turned exactly the matching test red naming the missing `COPY`, then reverted (`git status --short` clean)             |
| The same holds for a workspace removed from the downloader's `Dockerfile`                                     | `packages/core/test/image-closure.test.ts:318 "has its manifest and"` ✓ — verified by mutation: removed `resolvers`' runtime `dist` line from `tools/downloader/Dockerfile`, red with the exact missing line, reverted                                                                                                                                                                          |
| A workspace imported under `src` but undeclared in `package.json` fails, naming the package and the specifier | `packages/core/test/image-closure.test.ts:408 "a bare workspace specifier under"` ✓ — verified by mutation: removed `@planner/itinerary` from `tools/planner/api/package.json`; failed with `tools/planner/api/src/runs/orchestrator.ts imports @planner/itinerary, undeclared in @planner/api`, reverted                                                                                       |
...
| Both tools' Dockerfiles pass, or the fix ships in the same commit                                             | `packages/core/test/image-closure.test.ts:304 "has its manifest copied before"`, `packages/core/test/image-closure.test.ts:318 "has its manifest and"`, `packages/core/test/image-closure.test.ts:336 "neither list ships a workspace"`, `packages/core/test/image-closure.test.ts:408 "a bare workspace specifier under"` ✓ — unmodified worktree: `npm test -- --project core` is 10/10 green |
