...
| Done when                                                                                                                                        | Proof                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| ------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| One typed builder in `helpers/intakes.ts` is the only place in the planner's suites that hand-writes `INSERT INTO intakes`/`INSERT INTO answers` | `grep -rn "INSERT INTO (intakes \| answers)" tools/planner`→ only`api/test/helpers/intakes.ts:129 "INSERT INTO intakes (id, title"`and`api/test/helpers/intakes.ts:133 "INSERT INTO answers"`(the builder) plus the two documented exceptions the ticket's Log names:`intakes-store.test.ts:105-109 "not json"`(malformed-JSON case, can't be typed) and`migrations.test.ts@95c6403:125-129 "running twice changes nothing"`(schema test, no`App`/tree). This line says "asserted by reading the file, not by a scan," so verified by inspection rather than a test, as intended. ✓                                                                                                    |
| All three call sites use it, and each still asserts what it asserted before                                                                      | `saveStaleIntake`/`saveVersionOneRoadTrip`/`saveVersionTwoRoadTrip` now call `saveIntake` (`intakes-routes.test.ts:288 "function saveStaleIntake"`, `intakes-routes.test.ts:311 "function saveVersionOneRoadTrip"`, `intakes-routes.test.ts:375 "function saveVersionTwoRoadTrip"`); assertions unchanged — `discarded`, `answerRows`, `brief` and `updatedAt` checks at `intakes-routes.test.ts:335-356 "Neither can be named"` (v1↔v2), `intakes-routes.test.ts:393-408 "A discard here means"` (v2↔v3), `intakes-routes.test.ts:460 "second.intake.updatedAt"` (`expect(second.intake.updatedAt).toBe(NOW.toISOString())`, the stale-intake case) all read identically to `main`. ✓ |
| Answers typed against `@planner/contract`                                                                                                        | `helpers/intakes.ts:123 "treeVersion: number; answers: Answers"` — `saveIntake(app: App, intake: { id: string; treeVersion: number; answers: Answers })`, `Answers` imported from `@planner/contract`; values built through `answered()` which returns `Answer`. `npm run check` typechecks this under `tsconfig.tests.json` (verbose build log shows `intakes-routes.test.ts` as that project's newest input) — clean. ✓                                                                                                                                                                                                                                                              |
...
- **2026-09-12 — repo-39 folded in the first acceptance row's rendering defect.** The row's `grep` pattern carries a bare `|` inside a code span, and a GFM table splits a cell on every unescaped pipe, code spans included. The formatter had then padded that pipe as a column boundary, so the row rendered as a run of whitespace beside run-together prose. It is escaped now, with the pattern's text otherwise unchanged from the commit that first wrote it. Carried here from repo-37, which saw it and left it because it was not a citation defect.
