/**
 * Admission control: bounding one caller's rate, and everyone's simultaneous
 * cost.
 *
 * Lifted here from `tools/downloader/api/src/rate-limit.ts` when the planner
 * became the second real consumer — a plan run is a fan-out over a roster of
 * model calls, which is expensive enough to be a trivial denial of service in
 * exactly the way a browser probe is. The repo's rule is that shared code moves
 * to `packages/` on the second consumer rather than the first guess, and this is
 * that moment.
 *
 * **What did not move is the Fastify hook.** Refusing a request means throwing
 * that tool's `AppError` with that tool's `RATE_LIMITED` code and logging
 * through that tool's logger, none of which this package can know about. So each
 * `api` keeps a small `createRateLimitHook` of its own over the mechanism below.
 * The arithmetic is what was duplicated; the refusal never was.
 *
 * Two independent mechanisms, because they fail differently:
 *
 *  - **A token bucket per client** (`RateLimiter`). Bounds one caller's rate.
 *  - **A global concurrency gate** (`ConcurrencyGate`). Bounds everyone's
 *    simultaneous cost. A distributed flood passes every per-client bucket and
 ...
 * expensive endpoint is the difference between a limit and a suggestion. A
 * bucket refills continuously, so the worst case is exactly `capacity`.
 *
