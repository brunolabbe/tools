...
- **high** · `tools/downloader/api/src/egress-proxy.ts@95c6403:484-495` "This proxy exists to fetch a URL the user chose" (repo-44: the comment this finding quotes was rewritten in the round Gate 2 checks; the range holds its corrected text now) — the suppression
  ...
  `guard.assertAllowed`: removing both call sites (`tools/downloader/api/src/egress-proxy.ts:472` "await guard.assertAllowed(target)", `tools/downloader/api/src/egress-proxy.ts@95c6403:537` "await guard.assertAllowed(`https://${target}`)") fails 5 tests,
  ...
  `isBlockedAddress` rejection inside `createPinningLookup` (`dispatcher.ts@95c6403:163-168` "if (!guard.isExemptHost(hostname)) {")
...
- **verified, no defect** · the `net.connect` restraint — only `http.request` was
  ...
  (`tools/downloader/api/src/egress-proxy.ts:472` "await guard.assertAllowed(target)"/`tools/downloader/api/src/egress-proxy.ts@95c6403:537` "await guard.assertAllowed(`https://${target}`)"), so no functional gap results, and an unconfirmed suppression would
...
- **verified, no defect** · register argument — `grep -rn 'codeql\[' --include='*.ts' .`
  returns exactly one line, `egress-proxy.ts@95c6403:495` "codeql[js/request-forgery]", matching the ADR. The adr/003
