...
| #   | Finding                                                                                                                                                                                                                                        | Disposition                                                                                                                                                                                                                                               |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **med** · `package.json@20eb8ba:37` "// status" — the `"// status"` annotation still advertised `--write` as usable, one line above the `status` script. Invisible to `git grep 03-STATUS` because the ADR filename is lowercase `status-page` | **Fixed** — rewritten to describe what the script now does and to say `--write`'s pages were retired. The method half is the finding: the Log now records that the sweep needs a second pass over the _other names_                                       |
| 2   | **low** · `scripts/status.mjs@4fcd133:150` "file, dir: docs })" — `dir: docs` pushed onto every ticket, read by nothing since `statusPath` went, absent from the JSDoc return type, and serialised into `--json`                               | **Fixed** — removed (repo-44: the citation is pinned to `4fcd133`, `main` before this merged, the last reachable tree that still holds the line). It was `statusPath`'s input and died with it; `renderMarkdown` and `describeTicket` both key off `file` |
...
**Dropped by the reviewer, no action:** `tools/planner/CLAUDE.md:79` "The unit suite runs on every push in the repo-wide CI." ("the unit
