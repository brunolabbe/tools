...
- **2026-09-14 — the page was restructured bundle-first, which overturns this ticket's "do not reorder", on the owner's decision.** `docs/02-DEPLOYMENT.md` now does the host setup once for every tool it runs, then gives each tool a section of its own, then the volume migration. The downloader is no longer the worked example the signpost described, and `## Adding the second tool` is gone: its content is folded into `## Choose what this host runs`, steps 2 to 4 and `## The planner`. The owner chose this after being shown both rejections above, and neither holds as written any more. The delta section was rewritten rather than moved, so it cannot read incoherently above step 1. The line citations a reorder shifts were measured before anything was touched — 5 into this page and 3 into `README.md`, across four gated records — and pinned to `1835657`, where each still holds its anchor. The split rejection stands: nothing was split out, and the page stays at the root, now carrying in its own text the reason it must not move under a tool. `Done when` 1 to 3 were this ticket's acceptance at the time and are no longer true of the file; they are left as written.
