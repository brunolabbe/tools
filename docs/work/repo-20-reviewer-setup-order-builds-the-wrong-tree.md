...
| 2. explicit statement that only `npm run build` is order-dependent                             | `ticket-reviewer.md:33-36` "Only step 4 is strictly order-dependent" — verified by reading                                                                                                                      |
...
| 4. dispatching.md's stopgap clause retired, cites repo-20                                      | `dispatching.md:151-167` "The checkout comes before the build — a measurement, not a" — verified by reading + grep sweep, no dangling anchor references anywhere in the tree                                    |
...
- **Citations checked, not just trusted.** Every line the builder cited (`ticket-reviewer.md:22` "then `git checkout --detach <sha>` for the commit you were", `:25` "Confirm you are looking at the right tree", `:27` "Populate `node_modules` with", `:31` "4. `npm run build`.", `:33-36` "Only step 4 is strictly order-dependent", `:38-52` "Neither half fails loudly"; `dispatching.md@fdafd1a:151` "The checkout comes before the build — a measurement", `:153` "Nothing about this belongs in a gate prompt any more", `:163` "reordered that page to fetch"; `builder.md@fdafd1a:16-18` "the caller maps it here — reading") was opened and matches the claim made about it. `ticket-reviewer.md:25` "Confirm you are looking at the right tree" cites the "confirm tree" step rather than literally "detach", but it sits correctly between detach and farm in the reading order, so the citation supports the claim it's attached to.
