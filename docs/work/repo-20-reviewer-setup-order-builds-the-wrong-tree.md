...
| 5. builder.md reconciles "inherit" with explicit-model dispatch, one clause, no restated table | `builder.md:16-18` "the caller maps it here — reading" — verified by reading; premise (`.claude/skills/orchestrate-tickets/SKILL.md@3e58c32:84-86` "Then pass that model explicitly") checked and holds |
| 6. `npm run check` exits 0                                                                     | verified — ran it on the committed tree, exit 0                                                                                                                                                         |

- **Citations checked, not just trusted.** Every line the builder cited (`ticket-reviewer.md:22` "then `git checkout --detach <sha>` for the commit you were", `:25` "Confirm you are looking at the right tree", `:27` "Populate `node_modules` with", `:31` "4. `npm run build`.", `:33-36` "Only step 4 is strictly order-dependent", `:38-52` "Neither half fails loudly"; `dispatching.md:151` "The checkout comes before the build — a measurement", `:153` "Nothing about this belongs in a gate prompt any more", `:163` "reordered that page to fetch"; `builder.md:16-18` "the caller maps it here — reading") was opened and matches the claim made about it. `ticket-reviewer.md:25` "Confirm you are looking at the right tree" cites the "confirm tree" step rather than literally "detach", but it sits correctly between detach and farm in the reading order, so the citation supports the claim it's attached to.
