...
| Done when                                                                                      | Proof                                                                                                                                                                                                           |
| ---------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1. fetch→detach→farm→build→measure, single reading order, cannot build before checkout         | `ticket-reviewer.md@1bb63fa:20-31` "Set up in this order, and **all of it before any test or check**" — verified by reading; single ordered list under one heading                                              |
| 2. explicit statement that only `npm run build` is order-dependent                             | `ticket-reviewer.md@fdafd1a:33-36` "Only step 4 is strictly order-dependent" — verified by reading                                                                                                              |
| 3. both existing warnings survive the move, attached to their steps                            | `ticket-reviewer.md@1bb63fa:38-51` "Neither half fails loudly, and both fail into the same shape" — verified by reading; merged into one sequence per Build step 2's own instruction                            |
| 4. dispatching.md's stopgap clause retired, cites repo-20                                      | `dispatching.md@fdafd1a:151-167` "The checkout comes before the build — a measurement, not a" — verified by reading + grep sweep, no dangling anchor references anywhere in the tree                            |
| 5. builder.md reconciles "inherit" with explicit-model dispatch, one clause, no restated table | `builder.md@fdafd1a:16-18` "the caller maps it here — reading" — verified by reading; premise (`.claude/skills/orchestrate-tickets/SKILL.md@3e58c32:84-86` "Then pass that model explicitly") checked and holds |
| 6. `npm run check` exits 0                                                                     | verified — ran it on the committed tree, exit 0                                                                                                                                                                 |

- **Citations checked, not just trusted.** Every line the builder cited (`ticket-reviewer.md@fdafd1a:22` "then `git checkout --detach <sha>` for the commit you were", `:25` "Confirm you are looking at the right tree", `:27` "Populate `node_modules` with", `:31` "4. `npm run build`.", `:33-36` "Only step 4 is strictly order-dependent", `:38-52` "Neither half fails loudly"; `dispatching.md@fdafd1a:151` "The checkout comes before the build — a measurement", `:153` "Nothing about this belongs in a gate prompt any more", `:163` "reordered that page to fetch"; `builder.md@fdafd1a:16-18` "the caller maps it here — reading") was opened and matches the claim made about it. `ticket-reviewer.md@fdafd1a:25` "Confirm you are looking at the right tree" cites the "confirm tree" step rather than literally "detach", but it sits correctly between detach and farm in the reading order, so the citation supports the claim it's attached to.
