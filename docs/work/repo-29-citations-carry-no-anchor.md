...
| 3. No citation anchored to a non-distinctive fragment                     | **proven** — `scripts/test/citations-gate.test.ts:195 "an anchor that is not unique in its target fails the gate"` ✓, `scripts/test/citations.test.ts@64edce2:1795 "changes the exit code and not one citation line"` ✓ |
| 4. Evidence declarations cover what fails, and excuse nothing that passes | **proven** — `scripts/test/citations.test.ts@fdafd1a:1167 "exit 8 — 1 stale evidence declaration"` ✓                                                                                                                    |
