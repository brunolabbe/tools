...
| Hook fires, does not block                                                | `scripts/test/hooks.test.ts:68` "check-tree-grep warns on a tree-walking grep, and does not" ✓                                                                                                                                                                                                                                        |
| Stays quiet on the four already-correct spellings                         | `scripts/test/hooks.test.ts:93` "check-tree-grep stays quiet on the spellings that are" ✓                                                                                                                                                                                                                                             |
| Fires after a shell operator, not only at the start                       | `scripts/test/hooks.test.ts:74` "check-tree-grep fires after a shell operator, not only at" ✓                                                                                                                                                                                                                                         |
| Header carries the reasoning                                              | `scripts/test/hooks.test.ts:127` "check-tree-grep's header carries the reasoning, not a" ✓                                                                                                                                                                                                                                            |
| Siblings cited, not copied                                                | `scripts/test/hooks.test.ts:136` "check-tree-grep cites its siblings rather than copying them" ✓                                                                                                                                                                                                                                      |
| Wired into `.claude/settings.json`                                        | `scripts/test/hooks.test.ts@36be01b:134` "both hooks are wired into the PreToolUse Bash matcher" ✓ — `deny`/`allow`/`ask` byte-identical to `origin/main` at every tip reviewed; only delta anywhere is the added hook entry                                                                                                          |
| Paren-adjacent shape is silent                                            | `scripts/test/hooks.test.ts:110` "check-tree-grep stays quiet on the paren-adjacent shape" ✓                                                                                                                                                                                                                                          |
| `check-pr-title.sh` fixed, still does its job (both halves)               | `scripts/test/hooks.test.ts:180` "check-pr-title ignores a command that only mentions the phrase", `:189` "check-pr-title still rejects a title that is not a conventional commit", `:197` "check-pr-title still rejects an invocation with no inspectable title", `:275` "check-pr-title still reads a title out of a quoted span" ✓ |
...
- **Mutation-verified rather than accepted.** `if (false)` in place of the guard's condition fails exactly `scripts/test/commit-message.test.ts:304` "run as a process, a bad message is rejected" ("run as a process, a bad message is rejected"), with the other 26 tests in that file green. Restored, clean, 27/27.
- **The accepting-direction weakness is real, not a hedge.** The paired test at `scripts/test/commit-message.test.ts:312` "run as a process, a good message is accepted" stays green even against the same dead-entry-point mutation — a dead guard exits 0, which an accepting assertion cannot distinguish from a live one accepting valid input. Confirmed directly; the label on that test is honest.
