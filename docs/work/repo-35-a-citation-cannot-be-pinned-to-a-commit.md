...
- NFR: security — `makeTrees` resolves a pin via `execFileSync("git", ["rev-parse", ...])` with the rev pre-validated as 7–40 hex characters by the grammar before use, argument array, no shell ✓; performance — pin readers/resolvers memoised per distinct rev in `makeTrees`'s `Map` ✓; reliability — CI exposure checked directly: `.github/workflows/ci.yml`'s `check` job (which runs `citations-gate.mjs`) uses `fetch-depth: 0`, and the `test` job's shallow default checkout is not exposed because every rev/pin test builds its own self-contained fixture repo rather than reading the real checkout's history; maintainability — the six-site wording table and the narrow-hole bound for part 8/9's interaction are the "why, not what" documentation this repo's style calls for.
