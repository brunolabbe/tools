...
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1. `npm run status -- --ready` returns only tickets that can actually be started, proven by a test over a fixture tree with one startable ticket and one waiting on a decision | **proven** — `scripts/test/status.test.ts:1239-1244` "a ticket waiting on a decision is not ready, and the one beside it still is" (unit, `readyTickets` over a `repoWith` fixture) and `:1249-1258` "--ready lists the startable ticket and says on stderr what it withheld and why" (CLI, `--ready`'s stdout/stderr)                                                                                                                                                                                                  |
| 2. That test failed before the change, and the Log says so with its output                                                                                                     | **verified** — see "The 15-vs-11 reconstruction" below                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 3. `--ready` says what it withheld and why, rather than silently returning fewer rows                                                                                          | **proven** — `scripts/test/status.test.ts:1249-1257` "--ready lists the startable ticket and says on stderr what it withheld and why", `:1264-1269` "--ready withholding everything still says why", `:1277-1290` "the withheld notice is the whole account, and in-flight work is not part of it", and the widened `test.each` row at `:964` "[WITHHELD_REPO9, DANGLING]" (dependency-blocked case gains a second stderr line, asserted at `:978-979` "toEqual(onStderr)")                                             |
| 4. The decision above is recorded on this ticket with its answer and its reason                                                                                                | **proven** (documentation line, checked by inspection rather than a test) — this ticket's _Answered by the repo's owner — **A**, named `needs-decision`_ section                                                                                                                                                                                                                                                                                                                                                        |
| 5. If a `status` value was added, no ticket carries it in a commit earlier than the parser that accepts it                                                                     | **verified** — `git log origin/main..HEAD --oneline` shows exactly one commit (`424be56`) moving `STATUSES` (`scripts/status.mjs@91c18f9:69` "const STATUSES = ["), `repo-15`'s and `repo-16`'s frontmatter (`docs/work/repo-15-deny-list-does-not-protect-itself.md@91c18f9:6` "status: needs-decision", `docs/work/repo-16-suppression-does-not-dismiss.md@91c18f9:6` "status: needs-decision"), and the field table (`docs/01-TICKETS.md@91c18f9:95` "`needs-decision` · `ready` · `in-flight` · `done` ·") together |
...
**The 15-vs-11 reconstruction, in the permanent record.** Re-running the Log's Done-when-2 claim independently: I pulled `origin/main`'s `scripts/status.mjs` and swapped it into a checkout of this branch, then ran `npx vitest run scripts`. First attempt gave `15 failed | 121 passed (136)`, not the Log's `11 failed | 125 passed (136)`. Diagnosis, not assumption: three of the four extra failures came from tests that read the live `docs/work/` tree rather than a `repoWith` fixture (`scripts/test/status.test.ts@91c18f9:178` "every ticket in the repo parses, and its dependencies", `scripts/test/status.test.ts@91c18f9:196` "no ticket on the board is ready with a gate record already", `scripts/test/status.test.ts@91c18f9:560` "--tool with a name no tool has is a named failure" at the pre-amend line numbers) — they threw because `docs/work/repo-15-...md` and `docs/work/repo-16-...md` already carried `status: needs-decision` in my checkout while the parser I'd swapped in did not know that value. That is Done-when 5's own hazard — a ticket carrying the new value in a commit earlier than the parser — reproducing itself inside my reconstruction, because I had reverted only the script and not the two ticket files to match. Reverting `repo-15` and `repo-16`'s frontmatter to `origin/main`'s `status: ready` alongside the old parser reproduced the Log's number exactly: `11 failed | 125 passed (136)`. Under that exact reproduction, all three verbatim `FAIL` blocks quoted in the Log — the `validate` error on `pl-1-slug.md` for "a ticket waiting on a decision is open work, and its milestone has not started", the `expected '' to match /status\s+needs-decision/` for "--show ... does not call it unblocked", and `expected 1 to be +0` for "--ready lists the startable ticket ..." — match what I captured byte-for-byte. "After" state at the amended tip (`424be56`, including the low-finding test below): `137 passed (137)`. The red run is real and the Log's self-correction (first draft quoted an unobserved message, then the author swapped the old script back in, re-ran, and got `expected 1 to be +0`) is corroborated rather than contradicted.

**Gate finding closed.** I raised one low finding: `withheldFromReady`'s documented priority rule (`scripts/status.mjs:562-564` "The decision comes first when a ticket carries both", comment at `:562-563` "The decision comes first": "the decision comes first when a ticket carries both") — a ticket with both `needs-decision` status and an unmet dependency — was tested for `--show`'s closing line (`scripts/test/status.test.ts:1350-1357` "--show says both when a decision and a dependency are outstanding") but not for `withheldFromReady`/`--ready`'s stderr notice itself. The builder closed it: added `withheldFromReady reports the decision, not the dependency, when a ticket carries both` (`scripts/test/status.test.ts:1299-1310` "withheldFromReady reports the decision, not the dependency"), and I independently mutation-verified it — inverted the reason ternary's branch order in `scripts/status.mjs` (dependency check first, decision check second) and reran; the new test is the one that fails, with the message flipping from the needs-decision reason to `"waits on pl-2"`, exactly as the ternary's actual behavior would produce if the priority were wrong. Restored the correct code and reran: `137 passed (137)`. Closed by test, not dropped.

- **findings** · 1 returned by this gate's defect hunt, 1 carried, 1 closed by the builder's follow-up commit, 0 dropped.
- NFR: security n/a (no new URL, credential, or subprocess path touched) · performance n/a · reliability ✓ (`validate` still rejects an unknown status by construction, exercised at `scripts/status.mjs:453` "if (!STATUSES.includes(ticket.status))" via the fail-first checks above) · maintainability ✓ — `needs-decision` has exactly one home in source (`grep` for `STATUSES`/`.status ===`/`.status !==` across `scripts/` and `.claude/` returns only `scripts/status.mjs`), and the taxonomy comment at `scripts/status.mjs:61-76` "In lifecycle order, and `needs-decision` is first" documents the ordering rationale in place.
