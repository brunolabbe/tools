...
| …and `-- --ready` lists only tickets whose dependencies are all `done` | `scripts/test/status.test.ts:546` "ready means ready and unblocked — a ticket waiting on open" ✓ — `pl-3` waiting on ready `pl-2` is excluded                                                                                                |
| Every one of the repo's tickets parses, asserted over the real files   | `scripts/test/status.test.ts:178` "every ticket in the repo parses, and its dependencies" ✓ — reads `REPO`, not a fixture, and asserts the tool set against `readdirSync(tools/)`                                                            |
...
| A `03-STATUS.md` with no markers fails rather than being appended to   | `scripts/test/status.test.ts@022dfff:222` "a page with no markers is refused rather than appended to" ✓, and `:226` "markers the wrong way round are refused" for markers reversed                                                           |
| `npm run check` and `npm test` are green                               | **verified** — exit 0; 84 files / 1168 tests, unchanged from the base commit                                                                                                                                                                 |
| …and the generated regions survive `npm run format` unchanged          | `scripts/test/status.test.ts@022dfff:206` "writing the region leaves everything around it exactly as" ✓ for the surrounding-text guarantee, plus **verified**: both regions re-diffed byte-identical to `origin/main` after each format pass |
...
| 2   | `docs/02-DEPLOYMENT.md@1835657:351` "The scope of that, and the shared store" still cited the page for the limiter's scope | **Fixed** — consequence inline, links `dl-6`'s Log                                                |
...
| #   | Finding                                                                                                                                                                                   | Disposition                                                                                                                                                                                                                 |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **med** · `.env.example:58` said the proxy's policy is "the only thing" that bounds egress — false, and self-contradicted two lines later                                                 | **Fixed** — five words deleted. `ssrf.ts@95c6403:223` "if (!(ALLOWED_SCHEMES as readonly" and `:243` "if (net.isIP(literal) !== 0) {" do bound it in proxy mode                                                             |
| 2   | **med** · The new rule contradicts `.claude/skills/review-ticket/SKILL.md@1d3efff:79-82` "The subagent returns the `## Review` section as text, and the caller appends it", which is live | **Fixed** — the skill is the document that was wrong; steps 7, 8 and the section shape rewritten                                                                                                                            |
...
| 8   | **low** · `tools/planner/docs/03-STATUS.md@022dfff:21-23` "The reasoning is in" carries three sentences of narrative the page's own table says belongs in an ADR                          | **Fixed** — cut to the ADR link, matching the downloader's page (repo-44: the reviewed tree did not survive the squash merge, so the citation is pinned to the merge, where that link stands in the three sentences' place) |
...
| #   | Finding                                                                                                                                                                                                                                                                                                                                   | Disposition                                                                                                                                                                                                                                   |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | **med** · `.claude/skills/review-ticket/SKILL.md@022dfff:290` "Above `## Log`. On a ticket that has been through several rounds" still said "replacing a previous `## Review` — one gate per ticket", contradicting the one-subsection-per-gate rule added in the same round, and `01-TICKETS.md:368` "keeps **one subsection per gate**" | **Fixed** — eleven words deleted. A caller following the old sentence would have deleted every earlier gate, which is repo-1's own failure twice over                                                                                         |
...
| 3   | **low** · `.claude/skills/review-ticket/SKILL.md@022dfff:249` "So name all three" said "name both acts" four lines after step 8 grew to three                                                                                                                                                                                             | **Fixed** — "name all three, and keep the third of them honest"                                                                                                                                                                               |
| 4   | **low** · The acceptance table cites the `test(` line, not the assertion — the style `.claude/skills/review-ticket/SKILL.md@fdafd1a:239-244` "Cite the line of the assertion, not the line of the" rules out                                                                                                                              | **Declined** — every row is truthful and the citations resolve; recorded because this table is now the repo's worked example and the next one should cite the assertion                                                                       |
| 5   | **low** · Its last row cites `scripts/test/status.test.ts@022dfff:206` "writing the region leaves everything around it exactly as it was" for the `npm run format` guarantee; that test proves `replaceRegion` preserves text outside the region — the writer, not the formatter                                                          | **Declined** — the row's real proof is its `verified` half, the byte-identical re-diff after each format pass. The citation is weak rather than wrong                                                                                         |
