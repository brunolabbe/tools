...
**2026-09-12 — repointed two moved Review citations after repo-40's merge.**
`main`'s `check` job failed `citations-gate.mjs` (run 34721897038, on #217's
merge `8500411`): the two `databaseFile:` citations at record line 111 pointed
at `config.ts:228` (downloader) and `:222` (planner), but repo-40's #214 and
#215 shifted both to `:231` and `:230` respectively when they changed the
`trustProxy` import, after this ticket's #211 had last merged `main` (at
`b3b3786`, before #213/#214/#215 existed) — no textual conflict, so nothing
flagged it. Changed only those two line numbers; the anchor text
(`databaseFile:`), the verdict and every other word of the Review section are
untouched. The reviewer who wrote that record is no longer running, so this
mechanical re-resolve is mine rather than a re-gate.
