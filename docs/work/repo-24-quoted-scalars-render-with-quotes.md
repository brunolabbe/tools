...
| Done when                                                                      | Proof                                                                                                                                                                                                                                                                                                |
| ------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1. `parseFrontmatter` rejects a quoted `title`, message names file/line/key    | `scripts/test/status.test.ts:348-351` "a quoted title is a named failure, and the name carries" ✓ proven                                                                                                                                                                                             |
| 2. No rendering path emits a quote mark not in the title, asserted on `stdout` | `scripts/test/status.test.ts:435-445` "no rendering path emits a quote mark the title did not have", paired with `:450-459` "a title carrying quote marks of its own renders with" and `:463-473` "a title opening with a backtick needs no quoting, and" ✓ proven                                   |
| 3. A quoted `depends_on` entry never produces a `dangling-dependency`          | `scripts/test/status.test.ts:479-491` "a quoted depends_on entry is named at the parse and never", paired with `:508-516` "the same dependency unquoted is a sound board with no" (unquoted sibling, sound) ✓ proven                                                                                 |
| 4. `note` and `milestone` are covered too                                      | note `scripts/test/status.test.ts:404-408` "a quoted note is rejected too — it is the field the"; milestone `scripts/test/status.test.ts:413-419` "a quoted milestone is named at the parse rather than" ✓ proven                                                                                    |
| 5. `docs/01-TICKETS.md` states the quoting rule                                | `docs/01-TICKETS.md:113-129` "A value runs to the end of its line and is taken literally"; `command grep -n quot docs/01-TICKETS.md` → lines 102, 103, 109, exit 0, reproduced directly; also `scripts/test/status.test.ts:527-530` "the ticket format states the rule the parser enforces" ✓ proven |
