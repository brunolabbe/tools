...
- **behaviour identity** · The lifted function is byte-identical to what both tools had. `8d79d8e:tools/downloader/api/src/config.ts:318-325`, `8d79d8e:tools/planner/api/src/config.ts:310-317` and `packages/core/src/trust-proxy.ts:27 "export function trustProxy(raw: string | undefined): boolean | string"` through the end of its body, with `export ` stripped, all md5 to `da5e4fcc9a5a7536813e6d816d8f70e5`; `diff` exits 0 between all three, and the round-2 test commit left that source blob untouched. Confirmed at runtime too: main's body compared against both tools' built `loadApiConfig` and the core export over 42 inputs — every boolean spelling in four casings, near-misses, IPv4, CIDR, comma lists, IPv6, `loopback`, whitespace forms, `undefined` — giving `cases=42 mismatches=0` on each tool tip and on a trial merge of the two.
