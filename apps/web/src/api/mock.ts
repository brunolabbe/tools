 ...
 * schemas from `@downloader/shared`, folds its own state with the same reducer
...
import { AppError, ROUTES, createJobRequestSchema, probeRequestSchema } from "@downloader/shared";
import type {
  CreateJobRequest,
  ErrorCode,
  Job,
...
export function createMockClient(options: MockClientOptions = {}): ApiClient {
  ...
  return {
    ...
    async listJobs(): Promise<JobListResponse> {
      const jobs = [...runtimes.values()].map((runtime) => runtime.job);
